...
LzTextSprite.prototype.setMultiline = function(m) {
    ...
    if (m) {
        ...
        var whitespace = this.className === 'lzswfinputtextmultiline' ? 'pre-wrap' : 'nowrap';
        this.applyCSS('overflow', '', 'scrolldiv');
    }
    if (this._whiteSpace !== whitespace) {
        // update cached value
        this._whiteSpace = whitespace;
        this.scrolldiv.style.whiteSpace = whitespace;
    }
...
LzTextSprite.prototype.getTextDimension = function (dimension) {
  ...
  // set padding to 0 to cancel out padding on fields - see LPP-9268
  var style = ("padding:0px;overflow:visible;width:" + width + ";height:auto;" +
...
LzTextSprite.prototype.setSelectable = function (s) {
    // NOTE [2010-9-5 max] this _must_ be called at least once
    this.selectable = s;
    ...
    if (this.quirks['prevent_selection_with_onselectstart']) {
        var handler = s ? this.__onselectstartHandler : this.__cancelhandler;
        this.__LZdiv.onselectstart = handler;
    } else {
        var selectstyle = s ? 'text' : 'none'
        var stylename = LzSprite.__styleNames.userSelect;
        this.__LZdiv.style[stylename] = selectstyle;
    }
}

// custom __mouseEvent handler for text
LzTextSprite.prototype.$LzSprite$__mouseEvent = LzSprite.prototype.__mouseEvent;
LzTextSprite.prototype.__mouseEvent = function(eventname){
    // call super
    this.$LzSprite$__mouseEvent(eventname)

    //Debug.info('LzTextSprite.__mouseEvent', eventname, this.selectable, this);

    // Return true if selectable to ensure the default event happens
    return this.selectable;
}

LzTextSprite.prototype.__onselectstartHandler = function (e) {
    e = e || window.event;
    // prevent bubbling
    e.cancelBubble = true;
    return true;
...
LzTextSprite.prototype.setResize = function (r){
    this.resize = r;
    this.applyCSS('overflow', r ? '' : 'hidden', 'scrolldiv');
}
...
LzTextSprite.prototype.setWidth = function (w){
    ...
    var nw = this.$LzSprite$setWidth(w);
    // no change
    if (nw == null) return;

    // Calculate the width of the scrolldiv.  When setting the width of a text 
    // sprite, the container width wants to include the padding/gutter but 
    // when measuring text you want the accurate width of the text
    ...
    if (scrolldivcss !== this.__widthcss) {
      this.__widthcss = scrolldivcss;
      var scrolldiv = this.scrolldiv;
      ...
      if (this.scrolling) {
        ...
        var hp = cdim(this.height || 0);
        var wp = cdim(clipwidth);
        scrolldiv.style.clip = 'rect(0 ' + wp + ' ' + hp + ' 0)';
      }
    }
    return scrolldivwidth;
}
...
LzTextSprite.prototype.setHeight = function (h) {
    ...
    var nh = this.$LzSprite$setHeight(h);
    // no change
    if (nh == null) return;

    // Calculate the height of the scrolldiv. When setting the height of a 
    // text sprite, the container height wants to include the padding/gutter 
    // but when measuring text you want the accurate height of the text
    var ht = (h >= this.__hpadding ? h - this.__hpadding : 0);
