    ...
    return {
        ...
        setAttribute(n, v) {
            ...
            if (a)
                ...
                attributes.push({ name: n, value: String(v), line });
        },
    };
}
export function parseHtmlDialect(src) {
    let i = 0, line = 1;
    const n = src.length;
    const err = (msg) => { throw new HtmlDialectError(`${msg} (line ${line})`); };
    const advance = (to) => { for (; i < to; i++)
        if (src[i] === "\n")
            line++; };
    function skipDoctypeOrComment() {
        if (src.startsWith("<!--", i)) {
    ...
    function parseAttrs(el) {
        for (;;) {
            while (i < n && /\s/.test(src[i]))
                advance(i + 1);
            if (i >= n)
                err(`unterminated <${el.tagName.toLowerCase()}> tag`);
            ...
            if (src[i] === "=") {
                ...
                if (q === '"' || q === "'") {
                    ...
                    if (end < 0)
                        err(`unterminated attribute value for ${name}`);
    ...
    function parseElement() {
        ...
        if (!m)
            err("malformed start tag");
        const tag = m[0].toLowerCase();
        advance(i + m[0].length);
        const el = makeElem(tag, startLine);
        parseAttrs(el);
        if (src[i] === "/" && src[i + 1] === ">")
            advance(i + 2); // slash ignored (HTML) — still needs a close tag unless void
        else
            advance(i + 1); // consume '>'
        if (VOID.has(tag))
            return el;
        if (RAW_TEXT.has(tag)) {
            ...
            if (close < 0)
                err(`unterminated <${tag}> raw text`);
            const raw = src.slice(i, close);
            if (raw.length)
                el.childNodes.push({ nodeType: TEXT, nodeValue: raw, line: textLine });
            advance(close);
            const end = src.indexOf(">", i);
            if (end < 0)
        ...
        for (;;) {
            ...
            if (src.startsWith("</", i)) {
                ...
                if (closeName !== tag)
                    err(`mismatched </${closeName}>, expected </${tag}>`);
            ...
            const textLine = line;
            let j = i;
            while (j < n && !(src[j] === "<" && (src[j + 1] === "/" || src[j + 1] === "!" || /[a-zA-Z]/.test(src[j + 1] ?? ""))))
                j++;
            if (j === i)
                j++; // lone '<' — consume as text
    ...
    while (i < n) {
        if (src[i] === "<") {
            if (skipDoctypeOrComment())
                continue;
            if (/[a-zA-Z]/.test(src[i + 1] ?? "")) {
                tops.push(parseElement());
                continue;
            }
        }
        if (!/\s/.test(src[i]))
            err(`unexpected content at top level: ${JSON.stringify(src.slice(i, i + 12))}`);
        advance(i + 1);
    }
    return tops;
}
/** Breadth-first search for the <laszlo-app> element. */
export function findLaszloApp(tops) {
    const stack = [...tops];
    while (stack.length) {
        const el = stack.shift();
        if (el.tagName === "LASZLO-APP")
            return el;
