...
export function tsTypeOf(t) {
    switch (t) {
        case "number":
        case "numberExpression": return "number";
        case "size":
        case "sizeExpression": return "number | string";
        case "boolean":
        case "inheritableBoolean": return "boolean";
...
export function es4TsType(t, declared) {
    if (!t)
        return "any";
...
const SPECIAL_NAMES = { text: "LzText", inputtext: "LzInputText" };
/** Built-in tag -> emitted TS class name (null when not an emitted built-in). */
export function builtinTsName(tag) {
    if (!EMIT_ORDER.includes(tag))
        return null;
...
const VIEW_RELATIONAL = { subviews: "LzView[]" };
...
export function generateLfcDts(reflection) {
    ...
    const schemaNameFor = new Map(); // lowercased reflected name -> emitted name
    for (const tag of EMIT_ORDER)
        schemaNameFor.set(("lz" + tag).toLowerCase(), builtinTsName(tag));
    const reflected = reflection ? [...reflection.classes.values()] : [];
    const mergedInto = new Map(); // emitted schema name -> reflected class
    const standalone = [];
    const aliases = []; // [realName, emittedName] when spellings differ
    for (const rc of reflected) {
        const emitted = schemaNameFor.get(rc.name.toLowerCase());
        if (emitted) {
            mergedInto.set(emitted, rc);
            if (emitted !== rc.name)
                aliases.push([rc.name, emitted]);
        }
        else if (rc.name !== "LzEventable" && rc.name !== "LzDeclaredEvent")
            standalone.push(rc);
    ...
    const T = (t) => es4TsType(t, (n) => declaredNames.has(n));
    const methodSig = (m) => {
    ...
    for (const tag of EMIT_ORDER) {
        ...
        if (tag === "node")
            out.push(`  constructor(parent?: LzNode, attrs?: Record<string, any>);`);
        for (const [attr, sType] of Object.entries(cls.attrs)) {
            if (attr.startsWith("$") || attr === "with")
                continue;
        ...
        for (const ev of SCHEMA_EVENTS[tag] ?? [])
            if (!emitted.has(ev)) {
                out.push(`  ${ev}: LzDeclaredEvent;`);
                emitted.add(ev);
            }
        if (tag === "node")
        ...
        if (rc) {
            for (const v of rc.vars)
                if (!emitted.has(v.name)) {
                    out.push(`  ${v.isStatic ? "static " : ""}${v.name}: ${T(v.type)};`);
                    emitted.add(v.name);
                }
            for (const m of rc.methods)
                if (!emitted.has(m.name)) {
                    out.push(methodSig(m));
                    emitted.add(m.name);
                }
        }
        out.push("}", "");
    }
    for (const [real, emitted] of aliases)
        out.push(`type ${real} = ${emitted};`);
    if (aliases.length)
        out.push("");
    ...
    if (reflection) {
        ...
        for (const a of reflection.lzAssignments) {
            if (seen.has(a.prop) || !/^[A-Za-z_$][A-Za-z0-9_$]*$/.test(a.prop))
                continue;
            seen.add(a.prop);
    ...
    out.push("declare const canvas: LzCanvas;", "declare const Debug: any;", "declare var $debug: boolean;", "");
    return out.join("\n");
}
