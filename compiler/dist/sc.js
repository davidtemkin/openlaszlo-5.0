...
class Parser {
    constructor(toks) {
        this.pos = 0;
        this.lfc = false; // LFC library mode: tolerate AS3-only decls in dead branches (skip as no-ops)
    ...
    formalParams() {
        ...
        const names = [];
        const defaults = [];
        const types = []; // captured :Type annotations (reflection-only)
        let rest = null;
        while (!this.is(")")) {
...
export function setScProfile(v) { SC_PROFILE = v; }
...
export function meterEvent(lzp, now, name, getname, event) {
...
function collectFuncDecls(body) {
    ...
    const wS = (s) => {
        switch (s.s) {
            case "funcdecl":
                out.push(s);
                break; // does NOT descend into s.fn (own scope)
            case "block":
                s.body.forEach(wS);
                break;
            case "if":
...
function stripFuncDecls(body) {
    const sS = (s) => {
        switch (s.s) {
            ...
            case "while":
            case "dowhile":
            case "with": return { ...s, body: sS(s.body) };
            case "for": return { ...s, init: s.init && "s" in s.init ? sS(s.init) : s.init, body: sS(s.body) };
...
class Printer {
    nextGensym() { return "$lzsc$" + (this.gensym.n++).toString(36); }
    ...
    printAs3ClassDebug(n) {
        ...
        for (const m of n.members) {
            ...
            if (m.kind === "var") {
                ...
                target.push(renderDebugFuncNode(m.fn, userName, /*named*/ true, methodFile, m.fn.line ?? 0, "report", /*isMethod*/ !m.static, m.static ? undefined : as3, undefined, undefined, undefined, /*isStatic*/ m.static));
            }
        }
    ...
    joinStmtsInner(body) {
        ...
        for (const s of body) {
            ...
            if (this.dbg && this.joinDepth === 1 && s.file !== undefined)
                this.dfile = s.file;
            ...
            if (this.dbg) {
                ...
                if (ruleA && nestedFirstSuperActive && line !== (s.line ?? line)) {
                    ...
                    text = this.lnum(line, text, s.file); // the `(` token at funcLine−1
                    text = this.lnum(iifeOrigLine, text, s.file); // the statement directive at funcLine
                }
                else if (as3ClassLine != null) {
                    // Top-level Class.make: stream `#(classLine−1) #(classLine) #(classLine−1)`.
                    text = this.lnum(as3ClassLine - 1, text, s.file); // `Class` token (inner)
                    text = this.lnum(as3ClassLine, text, s.file); // `make` token (middle)
                    text = this.lnum(as3ClassLine - 1, text, s.file); // statement directive (outer)
                }
                    ...
                    text = this.lnum(line - 1, text, s.file); // first token (inner)
                    text = this.lnum(line, text, s.file); // statement own line (middle)
                    text = this.lnum(line - 1, text, s.file); // statement directive (outer)
                }
                else {
                    text = this.lnum(line, text, s.file);
                }
    ...
    throwsWrap(inner) {
        const catchBody = 'if(Error["$lzsc$isa"]?Error.$lzsc$isa($lzsc$e):$lzsc$e instanceof Error){' + NL +
    ...
    tryInlineSetAttribute(e) {
        ...
        const fragment = `if (! (${thisvar}.__LZdeleted )) {` +
...
export function compileLibraryProgram(rootSource, rootFile, resolveInclude, debug = false, backtrace = false, profile = false) {
    const parseFold = (src, file) => {
        try {
            ...
            if (e instanceof ScUnsupported)
                throw new ScUnsupported(`${e.message}  [in ${file}]`);
            throw e;
        }
    };
    const expandInclude = (path, stack) => {
        if (stack.includes(path))
            throw new ScUnsupported(`#include cycle: ${[...stack, path].join(" -> ")}`);
        const src = resolveInclude(path);
        if (src == null)
            throw new ScUnsupported(`#include not found: ${path}`);
        return expandList(parseFold(src, path), [...stack, path]);
    };
    const expandBranch = (s, stack) => {
        if (s.s === "include")
            return { s: "block", body: expandInclude(s.path, stack) };
        return expandOne(s, stack);
    };
    const expandOne = (s, stack) => {
        switch (s.s) {
            ...
            case "while":
            case "dowhile":
            case "with": return { ...s, body: expandBranch(s.body, stack) };
    ...
    const expandList = (stmts, stack) => {
        const out = [];
        for (const s of stmts) {
            if (s.s === "include")
                out.push(...expandInclude(s.path, stack));
            else
                out.push(expandOne(s, stack));
        }
        return out;
    };
    if (debug || profile) {
...
function renderDebugFuncNode(fn, userName, named, file, funcLine, catchKind = "report", isMethod = false, as3, propagateName, outerVars, asDecl, isStatic = false, insideFunc = false) {
    ...
    if (fn.userFunctionName !== undefined) {
    ...
    const hoistDecls = funcdecls.length
        ...
        : "";
    const hoistAssigns = funcdecls.length
        ? funcdecls.map((d) => A((d.line ?? funcLine) - 1) + printer.id(d.name) + " = " +
            renderDebugFuncNode(d.fn, d.name, /*named*/ true, file, d.line ?? funcLine, "report", false, undefined, undefined, undefined, undefined, false, /*insideFunc*/ true) + ";").join("\n")
        : "";
    const hoist = hoistDecls && hoistAssigns ? hoistDecls + "\n" + hoistAssigns : hoistDecls + hoistAssigns;
    const prologue = cases.length > 0
...
export function parseLibraryAst(rootSource, rootFile, resolveInclude) {
    const parseFold = (src, file) => {
        const p = new Parser(lex("#file " + file + "\n#line 1\n" + src, 1, undefined, true));
        p.lfc = true;
        return foldStmts(p.parseProgram());
    };
    const expand = (stmts, stack) => {
        const out = [];
        for (const s of stmts) {
            if (s.s === "include") {
                if (stack.includes(s.path))
                    throw new ScUnsupported(`#include cycle: ${[...stack, s.path].join(" -> ")}`);
                const src = resolveInclude(s.path);
                if (src == null)
                    throw new ScUnsupported(`#include not found: ${s.path}`);
                out.push(...expand(parseFold(src, s.path), [...stack, s.path]));
            }
            else if (s.s === "block")
                out.push({ ...s, body: expand(s.body, stack) });
            else
                out.push(s); // top-level classes/assignments are what reflection reads
        }
        return out;
    };
