...
export function emitObjectSpaced(entries) {
    const keys = Object.keys(entries).sort();
    if (keys.length === 0)
        return "{}";
    return "{" + keys.map((k) => `${emitKey(k)}: ${entries[k]}`).join(", ") + "}";
