...
function readLfcDts() {
    try {
        return readFileSync(fileURLToPath(new URL("../lfc.d.ts", import.meta.url)), "utf8");
    }
    catch {
        return generateLfcDts();
    }
}
...
export function checkApp(source, fileName) {
    ...
    host.getSourceFile = (name, langVer) => virtual.has(name) ? ts.createSourceFile(name, virtual.get(name), langVer, true) : origGet(name, langVer);
    ...
    for (const d of diags) {
        if (d.file?.fileName !== "__lzbodies.ts" || d.start == null)
            continue;
    ...
    for (const d of [...prog.getSyntacticDiagnostics(constrSf), ...prog.getSemanticDiagnostics(constrSf)]) {
        ...
        if (d.code === 2304 && span) {
            ...
            if (m && span.ownerMembers.includes(m[1]))
                continue; // with(this)-legal bare name
        }
