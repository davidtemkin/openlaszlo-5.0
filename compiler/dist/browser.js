...
export { validatorsEqual, isUpToDate, fnv1a, lookupKey, } from "./closure.js";
export { contentTag };
...
export async function compileInBrowser(mainUrl, o = {}) {
    ...
    const state = {
        map: new Map(), faults: new Set(), missing: new Set(),
    };
    ...
    let result = { js: "", unsupported: undefined };
    let passes = 0;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        if (toFetch.length === 0)
            break; // all faults already fetched (negative deps) → stable
