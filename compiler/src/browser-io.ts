...
import type { CompileOptions, ResourceInfo, FontInfo } from "./compile.js";

/** One fetched file, as captured by the driver loop. `bytes` is the raw content
 *  (for image dims), `text` its UTF-8 decoding (for source includes). */
export interface FetchedFile {
  bytes: Uint8Array;
  text: string;
}

/** The browser provider's view of the world: already-fetched files + the running
 *  fault set + the set of URLs KNOWN to be 404 (so a resolver skips them like
 *  node-io skips a missing candidate, instead of re-faulting forever or — worse —
 *  treating an empty placeholder as a real file). The driver owns all three. */
export interface BrowserIoState {
  map: Map<string, FetchedFile>;
  faults: Set<string>;
  missing: Set<string>;
  /** Called when a resolver actually USES a fetched URL (returns its content) — the
   *  closure-capture seam, mirroring node-io's `tracker?.file(abs)` inside `rd()`.
   *  This is what keeps the browser closure equal to Node's: a speculatively-fetched
   *  probe (e.g. a `.swf`→`.png` autoPng variant fetched in the same pass as the
   *  direct hit) is NOT in the closure unless a resolver returns it. Optional so the
   *  provider is usable without a tracker. */
...
function splitJ(s: string): string[] {
  const out: string[] = [];
  let start = 0;
  for (;;) {
    const end = s.indexOf("/", start);
    if (end === -1) { if (start > 0 || start < s.length) out.push(s.slice(start)); break; }
    out.push(s.slice(start, end));
    start = end + 1;
  }
  return out;
}
function normalizePath(path: string[]): void {
  for (let i = 0; i < path.length; i++) {
    const c = path[i];
    if (c === "." || c === "") { path.splice(i, 1); i--; }
    else if (c === ".." && i > 0) { path.splice(i - 1, 2); i -= 2; }
  }
}
function adjustRelativePath(p: string, source: string, dest: string): string {
  const sep = "/";
  if (p.endsWith(sep)) return p;
  if (source.endsWith(sep)) source = source.slice(0, -1);
  if (dest.endsWith(sep)) dest = dest.slice(0, -1);
  const sd = splitJ(source); const dd = splitJ(dest);
  normalizePath(sd); normalizePath(dd);
...
function getUserPathname(pathname: string, basePathnames: string[]): string {
...
function dirUrl(u: string): string {
...
function joinUrl(base: string, ref: string): string {
  ...
  const origin = m ? m[1] : "";
  let path = m ? (m[2] ?? "") : base;
  if (ref.startsWith("/")) path = ref;
  else path = (path.endsWith("/") ? path : path + "/") + ref;
...
function insertSubdir(u: string, sub: string): string {
...
export function browserOptions(args: {
  ...
  const want = (url: string): FetchedFile | null => {
    const hit = state.map.get(url);
    if (hit) { state.onUse?.(url); return hit; } // record the USED dep (closure capture)
    if (state.missing.has(url)) return null; // known 404 → skip (try next candidate)
  ...
  const PLACEHOLDER_RESOURCE: ResourceInfo = { width: 1, height: 1, ptype: "ar", relPath: "__lzc_pending__" };
  ...
  const relPathOf = (absUrl: string): { ptype: string; relPath: string } => {
    const abs = pathOf(absUrl);
    let ptype: string, prefix: string;
    if (abs.startsWith(appDirPath)) { ptype = "ar"; prefix = maxCommonPrefix(abs, appDirPath); }
    else { ptype = "sr"; prefix = lpsPath ? maxCommonPrefix(abs, lpsPath) : appDirPath; }
    let relPath = abs.startsWith(prefix) ? abs.slice(prefix.length) : abs;
    if (relPath[0] === "/") relPath = relPath.slice(1);
    return { ptype, relPath };
  };

  const resolveResource = (ref: string, originId?: string): ResourceInfo | null => {
    ...
    if (lps) baseDirs.push(lps + "/lps/components", lps + "/lps/fonts", lps + "/lps/lfc");
    pending = false;
    ...
    outer: for (const dir of baseDirs) {
      ...
      for (const cand of tries) {
        const f = want(cand);
        if (f) { foundUrl = cand; file = f; break outer; }
      }
    }
    if (!foundUrl || !file) return pending ? PLACEHOLDER_RESOURCE : null;
  ...
  const resolveFont = (src: string): FontInfo | null => {
    const cands: string[] = [];
    ...
    for (const cand of cands) {
      const f = want(cand);
      if (f) return relPathOf(cand);
    }
    return pending ? { ptype: "ar", relPath: "__lzc_pending__" } : null;
  };

  const frameInfo = (url: string): ResourceInfo | null => {
    const f = want(url);
    if (!f) return null;
  ...
  const resolveResourceFrames = (): ResourceInfo[] | null => null;

  const resolveInclude = (ref: string, fromId: string): { source: string; id: string } | null => {
    const base = fromId ? dirUrl(fromId) : appDir;
    const roots = ref.startsWith("/")
      ? (lps ? [joinUrl(lps + "/lps/components/", ref.slice(1))] : [])
      : [joinUrl(base + "/", ref)];
    if (lps && !ref.startsWith("/")) roots.push(joinUrl(lps + "/lps/components/", ref));
    const candidates = roots.flatMap((r) => [r, r + "/library.lzx"]);
    pending = false;
    for (const url of candidates) {
      const f = want(url);
      if (f) return { source: f.text, id: url };
    }
  ...
  const autoincludes: Record<string, string> = {};
  if (lps) {
    ...
    if (f) {
      for (const line of f.text.split("\n")) {
        ...
        if (m && !line.trimStart().startsWith("#")) autoincludes[m[1]] = m[2];
      }
    }
  }

  const resolveDatasetSrc = (ref: string, fromId?: string): string | null => {
    const base = fromId ? dirUrl(fromId) : appDir;
  ...
  const resolveScriptSrc = (ref: string, fromId: string): string | null => {
    const base = fromId ? dirUrl(fromId) : appDir;
    const cands = [joinUrl(base + "/", ref)];
    if (lps && !ref.startsWith("/")) cands.push(joinUrl(lps + "/lps/components/", ref));
    if (lps && ref.startsWith("/")) cands.push(joinUrl(lps + "/lps/components/", ref.slice(1)));
    pending = false;
    for (const url of cands) {
      const f = want(url);
      if (f) return f.text;
  ...
  const basePathnames: string[] = [appDirPath];
  if (lpsPath) basePathnames.push(
    lpsPath + "/lps/components",
    lpsPath + "/lps/fonts",
    lpsPath + "/lps/lfc",
  );
  const debugFileName = (id: string): string => {
