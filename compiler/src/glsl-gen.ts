...
import {
  ShaderType, INTRINSICS, OPERATORS, BUILTINS, constructorType, swizzleType,
  isRepeatedSwizzle, componentCount, Overload,
} from "./shader-table.js";

export interface ShaderUniform { name: string; lzType: "number" | "color" }
export interface ShaderHelper { name: string; params: { name: string; type: string }[]; ret: string; code: string; srcLine: number }
export interface ShaderlibPort {
  signatures: Record<string, { params: string[]; ret: string }>;   // "ns.fn"
  glslFor: (reachable: string[]) => string;
}
export interface GenInput {
  color: { code: string; srcLine: number };
  helpers: ShaderHelper[];
  uniforms: ShaderUniform[];
  shaderlib?: ShaderlibPort;
}
export interface ShaderProgram {
  glsl: string;
  uniforms: Array<{ name: string; glslType: "float" | "vec3" }>;
  usesTime: boolean; usesMouse: boolean;
}
export interface GenFinding { message: string; line: number }
export type GenResult = { ok: true; program: ShaderProgram } | { ok: false; findings: GenFinding[] };

const GLSL_UNIFORM: Record<string, "float" | "vec3"> = { number: "float", color: "vec3" };

const OP_NAME: Partial<Record<ts.SyntaxKind, string>> = {
...
const OP_GLSL: Record<string, string> = { __add: "+", __sub: "-", __mul: "*", __div: "/", __mod: "%" };
...
const normType = (t: string): ShaderType => (t === "number" ? "float" : t === "boolean" ? "bool" : (t as ShaderType));

function pickOverload(ols: Overload[], args: ShaderType[]): Overload | null {
...
export function generateShader(input: GenInput): GenResult {
  ...
  interface Ctx { sf: ts.SourceFile; srcLine: number; env: Map<string, ShaderType>[]; fnName: string; ret: ShaderType }
  const lineOf = (ctx: Ctx, node: ts.Node): number =>
    ctx.srcLine + ctx.sf.getLineAndCharacterOfPosition(node.getStart(ctx.sf)).line;
  const bad = (ctx: Ctx, node: ts.Node, message: string): "float" => {
    findings.push({ message, line: lineOf(ctx, node) });
    return "float"; // poison-resistant: keep going to collect more findings
  };
  const lookup = (ctx: Ctx, name: string): ShaderType | null => {
    for (let i = ctx.env.length - 1; i >= 0; i--) { const t = ctx.env[i].get(name); if (t) return t; }
    return null;
  };

  // ── typing + emission (one pass; emit returns GLSL text, typing accumulates) ──
  function expr(ctx: Ctx, e: ts.Expression): { t: ShaderType; s: string } {
    if (ts.isParenthesizedExpression(e)) { const r = expr(ctx, e.expression); return { t: r.t, s: `(${r.s})` }; }
    if (ts.isNumericLiteral(e)) {
      const txt = e.text;
      return { t: "float", s: /[.eE]/.test(txt) ? txt : txt + ".0" };
    }
    if (e.kind === ts.SyntaxKind.TrueKeyword) return { t: "bool", s: "true" };
    if (e.kind === ts.SyntaxKind.FalseKeyword) return { t: "bool", s: "false" };
    if (ts.isStringLiteralLike(e) || ts.isTemplateExpression(e)) return { t: bad(ctx, e, "strings are not expressible in GLSL"), s: "0.0" };
    if (ts.isArrayLiteralExpression(e)) return { t: bad(ctx, e, "arrays are not expressible in GLSL"), s: "0.0" };
    if (ts.isObjectLiteralExpression(e)) return { t: bad(ctx, e, "objects are not expressible in GLSL"), s: "0.0" };
    if (ts.isIdentifier(e)) {
      const name = e.text;
      const local = lookup(ctx, name);
      if (local) return { t: local, s: name };
      if (name in BUILTINS) {
    ...
    if (ts.isPropertyAccessExpression(e)) {
      ...
      if (e.expression.kind === ts.SyntaxKind.ThisKeyword) {
        ...
        if (!g) return { t: bad(ctx, e, `undeclared uniform 'this.${prop}' — declare <attribute name="${prop}">`), s: prop };
      ...
      if (!st) return { t: bad(ctx, e, `'.${prop}' is not a component of ${recv.t}`), s: `${recv.s}.${prop}` };
      return { t: st, s: `${recv.s}.${prop}` };
    }
    if (ts.isCallExpression(e)) return call(ctx, e);
    if (ts.isPrefixUnaryExpression(e)) {
      const r = expr(ctx, e.operand);
      if (e.operator === ts.SyntaxKind.MinusToken) {
        const ol = pickOverload(OPERATORS.__neg, [r.t]);
        if (!ol) return { t: bad(ctx, e, `unary '-' not defined on ${r.t}`), s: `-${r.s}` };
        return { t: ol.ret, s: `-${r.s}` };
      }
      if (e.operator === ts.SyntaxKind.ExclamationToken) {
        if (r.t !== "bool") return { t: bad(ctx, e, `'!' requires bool, got ${r.t}`), s: `!${r.s}` };
        return { t: "bool", s: `!${r.s}` };
      }
    ...
    if (ts.isConditionalExpression(e)) {
      ...
      if (c.t !== "bool") bad(ctx, e.condition, `ternary condition must be bool, got ${c.t}`);
      if (a.t !== b.t) bad(ctx, e, `ternary arms differ: ${a.t} vs ${b.t}`);
      return { t: a.t, s: `${c.s} ? ${a.s} : ${b.s}` };
    }
    if (ts.isBinaryExpression(e)) return binary(ctx, e);
    if (e.kind === ts.SyntaxKind.NewExpression) return { t: bad(ctx, e, "'new' is not expressible in GLSL"), s: "0.0" };
    return { t: bad(ctx, e, `unsupported expression (${ts.SyntaxKind[e.kind]})`), s: "0.0" };
  }

  function binary(ctx: Ctx, e: ts.BinaryExpression): { t: ShaderType; s: string } {
    ...
    if (k === ts.SyntaxKind.EqualsToken) {
      ...
      if (target.t !== rhs.t && target.ok) bad(ctx, e, `cannot assign ${rhs.t} to ${target.t}`);
    ...
    if (op) {
      ...
      if (at === "int" && (bt === "float" || componentCount(bt) > 1)) { as_ = `float(${as_})`; at = "float"; }
      if (bt === "int" && (at === "float" || componentCount(at) > 1)) { bs = `float(${bs})`; bt = "float"; }
      const ol = pickOverload(OPERATORS[op], [at, bt]);
      if (!ol) return { t: bad(ctx, e, `operand mismatch: ${a.t} ${OP_GLSL[op]} ${b.t}`), s: `${as_} ${OP_GLSL[op]} ${bs}` };
    ...
    if (EQ.has(k)) {
      if (componentCount(a.t) > 1) bad(ctx, e, `'=='/'!=' on vectors is not portable — use equal()/notEqual()`);
      const opTxt = (k === ts.SyntaxKind.EqualsEqualsEqualsToken || k === ts.SyntaxKind.EqualsEqualsToken) ? "==" : "!=";
      return { t: "bool", s: `${a.s} ${opTxt} ${b.s}` };
    }
    if (LOGIC.has(k)) {
      if (a.t !== "bool" || b.t !== "bool") bad(ctx, e, `logical operator needs bools, got ${a.t}/${b.t}`);
  ...
  function assignTarget(ctx: Ctx, e: ts.Expression): { t: ShaderType; s: string; ok: boolean } {
    if (ts.isIdentifier(e)) {
      const t = lookup(ctx, e.text);
      if (!t) return { t: bad(ctx, e, `assignment to unresolved '${e.text}'`), s: e.text, ok: false };
    ...
    if (ts.isPropertyAccessExpression(e)) {
      ...
      const st = swizzleType(recv.t, prop);
      if (!st) return { t: bad(ctx, e, `'.${prop}' is not a component of ${recv.t}`), s: `${recv.s}.${prop}`, ok: false };
      return { t: st, s: `${recv.s}.${prop}`, ok: true };
    }
    return { t: bad(ctx, e, "unsupported assignment target"), s: "0.0", ok: false };
  }

  function call(ctx: Ctx, e: ts.CallExpression): { t: ShaderType; s: string } {
    ...
    if (ts.isPropertyAccessExpression(e.expression) && ts.isIdentifier(e.expression.expression)) {
      ...
      if (input.shaderlib && !lookup(ctx, ns) && !(ns in BUILTINS)) {
        const key = `${ns}.${fn}`;
        const sig = input.shaderlib.signatures[key];
        if (!sig) return { t: bad(ctx, e, `unknown shaderlib function '${key}'`), s: "0.0" };
        const args = e.arguments.map((a) => expr(ctx, a));
        if (args.length !== sig.params.length)
          bad(ctx, e, `'${key}' expects ${sig.params.length} args, got ${args.length}`);
        else args.forEach((a, i) => { if (a.t !== normType(sig.params[i] as string)) bad(ctx, e.arguments[i], `'${key}' arg ${i + 1}: expected ${sig.params[i]}, got ${a.t}`); });
        usedLib.add(key);
    ...
    if (VEC_CTORS.has(name)) {
      const r = constructorType(name as ShaderType, args.map((a) => a.t));
      if (typeof r !== "string") return { t: bad(ctx, e, `${name}(): ${r.error}`), s: `${name}(${args.map((a) => a.s).join(", ")})` };
      return { t: r, s: `${name}(${args.map((a) => a.s).join(", ")})` };
    }
    const intr = INTRINSICS[name];
    if (intr) {
      const ol = pickOverload(intr, args.map((a) => a.t));
      if (!ol) return { t: bad(ctx, e, `no overload of ${name}(${args.map((a) => a.t).join(", ")})`), s: `${name}(${args.map((a) => a.s).join(", ")})` };
      return { t: ol.ret, s: `${name}(${args.map((a) => a.s).join(", ")})` };
    }
    const helper = helperSigs.get(name);
    if (helper) {
      if (name === ctx.fnName) bad(ctx, e, `recursion ('${name}' calls itself) is illegal GLSL`);
      if (args.length !== helper.params.length) bad(ctx, e, `'${name}' expects ${helper.params.length} args, got ${args.length}`);
      else args.forEach((a, i) => { if (a.t !== helper.params[i]) bad(ctx, e.arguments[i], `'${name}' arg ${i + 1}: expected ${helper.params[i]}, got ${a.t}`); });
  ...
  function stmt(ctx: Ctx, s: ts.Statement, out: string[], indent: string): void {
    if (ts.isVariableStatement(s)) {
      for (const d of s.declarationList.declarations) {
        if (!ts.isIdentifier(d.name)) { bad(ctx, d, "destructuring is not expressible in GLSL"); continue; }
        const declared = d.type ? d.type.getText(ctx.sf) : null;
        if (declared && !DIALECT_TYPES.has(declared)) { bad(ctx, d, `type '${declared}' is outside the shader dialect`); continue; }
        if (!d.initializer) { bad(ctx, d, "declarations need initializers"); continue; }
        const r = expr(ctx, d.initializer);
        const t = declared ? normType(declared) : r.t;
        if (declared && normType(declared) !== r.t) bad(ctx, d, `cannot initialize ${declared} with ${r.t}`);
    ...
    if (ts.isReturnStatement(s)) {
      if (!s.expression) { bad(ctx, s, "return needs a value"); return; }
      const r = expr(ctx, s.expression);
      if (r.t !== ctx.ret) bad(ctx, s, `${ctx.fnName}() must return ${ctx.ret}, got ${r.t}`);
      out.push(`${indent}return ${r.s};`);
      return;
    }
    if (ts.isIfStatement(s)) {
      const c = expr(ctx, s.expression);
      if (c.t !== "bool") bad(ctx, s.expression, `if condition must be bool, got ${c.t}`);
    ...
    if (ts.isForStatement(s)) {
      ...
      if (!header) { bad(ctx, s, "only canonical int-counter for loops are expressible (for (let i = 0; i < N; i++))"); return; }
    ...
    if (ts.isWhileStatement(s) || ts.isDoStatement(s)) { bad(ctx, s, "'while' loops are not expressible (GLSL ES 1.00)"); return; }
    if (ts.isBlock(s)) { block(ctx, s, out, indent); return; }
    if (ts.isTryStatement(s)) { bad(ctx, s, "'try' is not expressible in GLSL"); return; }
    bad(ctx, s, `unsupported statement (${ts.SyntaxKind[s.kind]})`);
  }

  function block(ctx: Ctx, s: ts.Statement, out: string[], indent: string): void {
    ctx.env.push(new Map());
    if (ts.isBlock(s)) for (const st of s.statements) stmt(ctx, st, out, indent);
    else stmt(ctx, s, out, indent);
    ctx.env.pop();
  }

  function genFunction(name: string, params: { name: string; type: ShaderType }[], ret: ShaderType,
                       code: string, srcLine: number): string {
    const sf = ts.createSourceFile("b.ts", code, ts.ScriptTarget.ES2020, true);
    const ctx: Ctx = { sf, srcLine, env: [new Map(params.map((p) => [p.name, p.type]))], fnName: name, ret };
    const out: string[] = [];
    for (const s of sf.statements) stmt(ctx, s, out, "  ");
  ...
  while (usedHelpers.size !== prev) {
    ...
    for (const h of input.helpers) {
      if (!usedHelpers.has(h.name) || emitted.has(h.name)) continue;
      const badParam = h.params.find((p) => !DIALECT_TYPES.has(p.type));
      if (badParam) { findings.push({ message: `helper '${h.name}' param '${badParam.name}': type '${badParam.type}' outside the dialect`, line: h.srcLine }); continue; }
  ...
  for (const h of input.helpers) if (emitted.has(h.name)) helperFns.push(emitted.get(h.name)!);

  if (findings.length) return { ok: false, findings };

  const parts: string[] = ["precision mediump float;"];
  if (usedBuiltins.has("uv")) parts.push("varying vec2 uv;");
  if (usedBuiltins.has("time")) parts.push("uniform float time;");
  if (usedBuiltins.has("mouse")) parts.push("uniform vec2 mouse;");
  if (usedBuiltins.has("size")) parts.push("uniform vec2 size;");
  for (const [name, g] of usedUniforms) parts.push(`uniform ${g} ${name};`);
  if (usedLib.size && input.shaderlib) parts.push(input.shaderlib.glslFor([...usedLib]));
...
export function rewriteOperators(code: string): { code: string } {
  ...
  function textOf(n: ts.Node): string {
    ...
    for (const sp of inner) s = s.slice(0, sp.start - base) + sp.text + s.slice(sp.end - base);
    return s;
  }
  ...
  let out = code;
  for (const sp of outer) out = out.slice(0, sp.start) + sp.text + out.slice(sp.end);
  if (out.split("\n").length !== code.split("\n").length)
