...
export function buildStylesheetProgram(cssText: string, debugFile?: string): string | null {
...
interface Rule {
  selector: string; // JS object/array literal for the selector
  props: string; // JS object literal for the properties
}
...
function parseRules(css: string): Rule[] {
  ...
  while (i < css.length) {
    ...
    if (open < 0) {
      if (css.slice(i).trim() !== "") throw new CssUnsupported("trailing CSS text");
      break;
    }
    const close = css.indexOf("}", open);
    if (close < 0) throw new CssUnsupported("unbalanced braces");
...
function buildSelector(sel: string): string {
  // Descendant combinator: whitespace between simple selectors (not inside []).
  const simples = splitSimples(sel);
  if (simples.length === 0) throw new CssUnsupported(`empty selector`);
  if (simples.length === 1) return buildSimple(simples[0]);
...
function splitSimples(sel: string): string[] {
...
function buildSimple(sel: string): string {
  ...
  if (rest === "") {
    ...
    const map: Record<string, string> = { s: "1" };
    if (element && element !== "*") map.t = jsString(element);
    return emitMap(map);
  }
...
function buildCondition(cond: string, element?: string): string {
  // #id — specificity 100; the element part is ignored (matches the oracle).
  let m = /^#([\w-]+)$/.exec(cond);
  if (m) return emitMap({ i: jsString(m[1]), s: "100" });

  // .class — styleclass attribute, ~= match, specificity 10 (+1 with element).
  m = /^\.([\w-]+)$/.exec(cond);
  if (m) {
    const map: Record<string, string> = { a: jsString("styleclass"), v: jsString(m[1]), m: jsString("~=") };
  ...
  if (m) {
    ...
    const map: Record<string, string> = { a: jsString(attr) };
    if (rawVal != null) map.v = jsString(unquote(rawVal));
    if (op === "~=") map.m = jsString("~=");
    else if (op === "|=") map.m = jsString("|=");
    else if (op && op !== "=") throw new CssUnsupported(`attribute operator ${op}`);
    return finishAttr(map, 10, element);
  }
...
function finishAttr(map: Record<string, string>, spec: number, element?: string): string {
  if (element && element !== "*") {
...
function buildProperties(body: string): string {
  const map: Record<string, string> = {};
  for (const decl of body.split(";")) {
    const d = decl.trim();
    if (d === "") continue;
    const idx = d.indexOf(":");
    if (idx < 0) throw new CssUnsupported(`CSS declaration: ${d}`);
    const name = d.slice(0, idx).trim();
    const value = d.slice(idx + 1).trim();
    if (!/^-?[a-zA-Z][\w-]*$/.test(name)) throw new CssUnsupported(`CSS property name: ${name}`);
...
function cssValueToJs(value: string): string {
  if (/!\s*important\s*$/i.test(value)) throw new CssUnsupported(`!important`);
  ...
  if (/^[a-zA-Z_][\w-]*$/.test(value)) return `new LzStyleIdent('${value}')`;
  throw new CssUnsupported(`CSS value: ${value}`);
}

/** CSSHandler.getRGBString: three int/percentage components → `0xRRGGBB`. */
function rgbToHex(args: string): string {
  const parts = args.split(",").map((s) => s.trim());
  if (parts.length !== 3) throw new CssUnsupported(`rgb() args: ${args}`);
  let hex = "0x";
  for (const p of parts) {
    let n: number;
    const pm = /^(\d+)%$/.exec(p);
    if (pm) n = Math.round(Math.min((parseInt(pm[1], 10) * 255) / 100, 255));
    else if (/^\d+$/.test(p)) n = parseInt(p, 10);
    else throw new CssUnsupported(`rgb() component: ${p}`);
...
function emitMap(map: Record<string, string>): string {
