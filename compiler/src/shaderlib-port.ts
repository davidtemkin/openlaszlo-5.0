...
import { generateLibFunction, GenFinding } from "./glsl-gen.js";
...
function parseLib(ns: string, src: string): LibFunction[] {
  const sf = ts.createSourceFile(ns + ".ts", src, ts.ScriptTarget.ES2020, true);
  const fns: LibFunction[] = [];
...
let cached: Shaderlib | null = null;

export function loadShaderlib(sources: Record<string, string> = SHADERLIB_SOURCES): Shaderlib {
  if (cached && sources === SHADERLIB_SOURCES) return cached;
  const namespaces = Object.keys(sources);
  const all: LibFunction[] = [];
  for (const ns of namespaces) all.push(...parseLib(ns, sources[ns]));

  const signatures: Record<string, { params: string[]; ret: string }> = {};
  for (const f of all) signatures[`${f.ns}.${f.name}`] = { params: f.params.map((p) => p.type), ret: f.ret };

  const findings: GenFinding[] = [];
  const glsl = new Map<string, { text: string; deps: string[] }>();
  for (const f of all) {
  ...
  const lib: Shaderlib = {
    ...
    glslFor(reachable: string[]): string {
      ...
      for (const k of reachable) visit(k);
      return out.join("\n");
    },
  };
  if (sources === SHADERLIB_SOURCES) cached = lib;
  return lib;
}
