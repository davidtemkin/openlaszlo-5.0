...
function escLen(s: string, j: number): number {
  const c = s[j + 1];
  if (c === "x") return 4;
  if (c === "u") return 6;
  return 2;
}
function unescapeChar(s: string, j: number): string {
  const c = s[j + 1];
  switch (c) {
    case "n": return "\n";
    case "r": return "\r";
...
type Node =
  | { k: "num"; raw: string }
  | { k: "str"; v: string }
  | { k: "id"; name: string; line?: number } // line = identifier token line (backtrace noteCallSite on checked free refs); inert otherwise
  | { k: "this" }
  | { k: "super" }
  | { k: "lit"; v: string } // true/false/null
  | { k: "member"; o: Node; p: string }
  | { k: "index"; o: Node; i: Node }
  | { k: "call"; c: Node; args: Node[]; line?: number } // line = call-expression start token (oracle node.beginLine, for backtrace noteCallSite)
  | { k: "new"; c: Node; args: Node[]; line?: number }
  | { k: "unary"; op: string; e: Node; prefix: boolean; line?: number } // line = the operand's source line (postfix ++/-- on a free id → debug IIFE expansion)
  | { k: "bin"; op: string; l: Node; r: Node }
  | { k: "logic"; op: string; l: Node; r: Node }
  | { k: "assign"; op: string; l: Node; r: Node }
...
type Stmt =
  | { s: "expr"; e: Node }
  | { s: "var"; decls: { name: string; init: Node | null }[] }
  | { s: "return"; e: Node | null }
  | { s: "if"; c: Node; t: Stmt; e: Stmt | null }
  | { s: "block"; body: Stmt[] }
  | { s: "while"; c: Node; body: Stmt }
  | { s: "dowhile"; c: Node; body: Stmt }
  | { s: "with"; c: Node; body: Stmt }
  | { s: "funcdecl"; name: string; fn: Extract<Node, { k: "func" }> }
  | { s: "for"; init: Stmt | Node | null; test: Node | null; upd: Node | null; body: Stmt }
  | { s: "forin"; varName: string | null; lhs: Node; obj: Node; body: Stmt }
  | { s: "switch"; disc: Node; cases: { test: Node | null; body: Stmt[]; line?: number; file?: string }[] }
  | { s: "empty" }
  | { s: "break" }
  | { s: "continue" }
  | { s: "throw"; e: Node }
...
function makeIsExpr(a: Node, b: Node, line?: number): Node {
  return {
    k: "cond",
...
function isSuperCallExpr(e: Node): boolean {
  if (e.k !== "call") return false;
  const c = e.c;
  if (c.k === "super") return true;
  if (c.k === "member" && c.o.k === "super") return true;
  if (c.k === "member" && (c.p === "call" || c.p === "apply") &&
      c.o.k === "member" && c.o.o.k === "super") return true;
  return false;
}

/** True iff `s` is a control (compound) statement — one with a controlled body
 *  (if/loop/with/try/switch/block). NOT recursing into it. */
function isControlStmt(s: Stmt): boolean {
  switch (s.s) {
    case "block": case "if": case "while": case "dowhile":
    case "with": case "for": case "forin": case "try": case "switch":
      return true;
...
function predecessorTriggersRuleA(s: Stmt, endsBrace: boolean): boolean {
  // Only an else-less `if` can suppress the quirk.
  if (s.s !== "if" || s.e) return true;
  const then = s.t;
  if (then.s !== "block") {
    ...
    return (then as any).line === (s as any).line;
  }
  if (!endsBrace) return false;
  ...
  return last ? !(isControlStmt(last) && (last as any).endsBrace === true) : false;
}
...
class Parser {
  ...
  eat(t: string): Tok {
    if (this.peek().t !== t) throw new ScUnsupported(`parse: expected ${t}, got ${this.peek().t} '${this.peek().v}'`);
    return this.next();
  }
  is(t: string): boolean { return this.peek().t === t; }
  ...
  statement(): Stmt {
    ...
    if ((s as any).endLine === undefined) (s as any).endLine = this.toks[this.pos - 1]?.line ?? line;
    ...
    if ((s as any).endsBrace === undefined)
      (s as any).endsBrace = this.toks[this.pos - 1]?.t === "}";
    ...
    if ((s as any).superQuirkPredecessor === undefined) {
      const endsBrace = (s as any).endsBrace as boolean;
      (s as any).superQuirkPredecessor = predecessorTriggersRuleA(s, endsBrace);
      ...
      (s as any).singleLineBlockIf = s.s === "if" && !(s as any).e
        && (s as any).line === (s as any).endLine && endsBrace;
    }
    return s;
  }
  ...
  statementInner(): Stmt {
    ...
    if (t === "id") {
      ...
      let k = 0;
      while (this.peek(k).t === "id" && CLASS_MODIFIERS.has(this.peek(k).v)) k++;
    ...
    if (t === "while") {
      this.next(); this.eat("("); const c = this.expression(); this.eat(")");
      return { s: "while", c, body: this.statement() };
    }
    if (t === "do") {
      this.next();
      const body = this.statement();
      this.eat("while"); this.eat("("); const c = this.expression(); this.eat(")");
      this.semi();
    ...
    if (t === "throw") {
      this.next(); const e = this.expression(); this.semi();
      return { s: "throw", e };
    }
    if (t === "try") {
      this.next();
      ...
      if (this.is("catch")) {
        ...
        this.next(); this.eat("(");
        param = this.eat("id").v; this.skipTypeAnnotation();
        this.eat(")");
        handler = this.block();
      }
    ...
    if (t === "function") {
      ...
      const fn = this.functionExpr() as Extract<Node, { k: "func" }>;
      if (fn.name) return { s: "funcdecl", name: fn.name, fn };
      this.semi();
      return { s: "expr", e: fn };
    }
    if (t === "for") return this.forStmt();
    if (t === "switch") return this.switchStmt();
    if (t === "break") { this.next(); this.semi(); return { s: "break" }; }
    if (t === "continue") { this.next(); this.semi(); return { s: "continue" }; }
    const e = this.expression();
    this.semi();
    return { s: "expr", e };
  }
  ...
  classDecl(): Stmt {
    ...
    while (this.is("id") && CLASS_MODIFIERS.has(this.peek().v)) this.next(); // dynamic/final/…
    const xtor: "Class" | "Mixin" = this.next().v === "mixin" ? "Mixin" : "Class"; // `class`/`mixin`
    const name = this.eat("id").v;
    let sup: string | null = null;
    if (this.is("id") && this.peek().v === "extends") {
      this.next();
      sup = this.eat("id").v;
      while (this.is(".")) { this.next(); sup += "." + this.eat("id").v; }
    }
    ...
    if (this.is("with")) { // `with` is a lexer keyword token
      ...
      do {
        ...
        while (this.is(".")) { this.next(); mn += "." + this.eat("id").v; }
        mixins.push(mn);
  ...
  ifStmt(): Stmt {
    this.eat("if"); this.eat("("); const c = this.expression(); this.eat(")");
    const t = this.statement();
    let e: Stmt | null = null;
    let elseLine: number | undefined;
    if (this.is("else")) { elseLine = this.peek().line; this.next(); e = this.statement(); }
    return { s: "if", c, t, e, elseLine } as Stmt;
  }
  forStmt(): Stmt {
    this.eat("for"); this.eat("(");
    let init: Stmt | Node | null = null;
    if (this.is(";")) this.next();
    else if (this.is("var")) {
      ...
      do {
        const name = this.eat("id").v;
        this.skipTypeAnnotation();
        let dinit: Node | null = null;
        if (this.is("=")) { this.next(); dinit = this.assign(); }
    ...
    this.eat(";");
    let upd: Node | null = null;
    if (!this.is(")")) upd = this.expression();
    this.eat(")");
    return { s: "for", init, test, upd, body: this.statement() };
  }
  switchStmt(): Stmt {
    this.eat("switch"); this.eat("("); const disc = this.expression(); this.eat(")");
    this.eat("{");
    const cases: { test: Node | null; body: Stmt[]; line?: number; file?: string }[] = [];
    while (!this.is("}")) {
      let test: Node | null = null;
      const labelTok = this.peek();
      if (this.is("case")) { this.next(); test = this.expression(); }
  ...
  commaTail(head: Node): Node {
    const es = [head];
    while (this.is(",")) { this.next(); es.push(this.assign()); }
    return { k: "seq", es };
  }
  assign(noIn = false): Node {
  ...
  cond(noIn = false): Node {
    ...
    if (this.is("?")) {
      this.next(); const t = this.assign(); this.eat(":"); const f = this.assign();
      return { k: "cond", c: e, t, f };
    }
    return e;
  }
  binary(minPrec: number, noIn = false): Node {
    let left = this.unary();
    for (;;) {
  ...
  unary(): Node {
    ...
    if (t === "++" || t === "--") { const line = this.peek().line; this.next(); return { k: "unary", op: t, e: this.unary(), prefix: true, line }; }
    return this.postfix();
  }
  postfix(): Node {
    const opTokLine = this.peek().line;
    let e = this.callMember();
    if (this.is("++") || this.is("--")) { const op = this.next().t; e = { k: "unary", op, e, prefix: false, line: opTokLine }; }
    return e;
  }
  callMember(): Node {
    ...
    for (;;) {
      if (this.is(".")) { this.next(); e = { k: "member", o: e, p: this.eat("id").v }; }
      else if (this.is("[")) { this.next(); const i = this.expression(); this.eat("]"); e = { k: "index", o: e, i }; }
      else if (this.is("(")) { e = { k: "call", c: e, args: this.argList(), line: startLine }; }
      else break;
    }
    return e;
  }
  callMemberNoCall(): Node {
    let e = this.primary();
    for (;;) {
      if (this.is(".")) { this.next(); e = { k: "member", o: e, p: this.eat("id").v }; }
      else if (this.is("[")) { this.next(); const i = this.expression(); this.eat("]"); e = { k: "index", o: e, i }; }
  ...
  argList(): Node[] {
    this.eat("(");
    const args: Node[] = [];
    while (!this.is(")")) { args.push(this.assign()); if (this.is(",")) this.next(); else break; }
    this.eat(")");
    return args;
  }
  primary(): Node {
    const t = this.peek();
    switch (t.t) {
      case "num": this.next(); return { k: "num", raw: t.v };
      case "str": this.next(); return { k: "str", v: t.v };
      case "id": this.next(); return { k: "id", name: t.v, line: t.line };
      case "this": this.next(); return { k: "this" };
      case "super": this.next(); return { k: "super" };
      case "function": return this.functionExpr();
      case "true": case "false": case "null": this.next(); return { k: "lit", v: t.t };
      ...
      case "(": { this.next(); const e = this.expression(); this.eat(")"); return e; }
      case "[": {
        this.next(); const els: Node[] = [];
        while (!this.is("]")) { els.push(this.assign()); if (this.is(",")) this.next(); else break; }
        this.eat("]"); return { k: "array", els };
      }
      case "{": {
        this.next(); const props: { key: string; keyKind: "str" | "num" | "id"; computed: boolean; v: Node }[] = [];
        while (!this.is("}")) {
          let key: string;
          let keyKind: "str" | "num" | "id";
          const kt = this.peek();
          if (kt.t === "str") { this.next(); key = kt.v; keyKind = "str"; }
          else if (kt.t === "num") { this.next(); key = kt.v; keyKind = "num"; }
          else { this.next(); key = kt.v; keyKind = "id"; } // id or keyword as key
          this.eat(":");
...
// ---------- Local renamer ($0,$1,…) ----------
function collectLocals(params: string[], body: Stmt[]): Map<string, string> {
  const order: string[] = [...params];
  const seen = new Set(order);
  const addName = (n: string) => { if (!seen.has(n)) { seen.add(n); order.push(n); } };
  const walkS = (st: Stmt) => {
    switch (st.s) {
      case "var": for (const d of st.decls) addName(d.name); break;
      case "funcdecl": addName(st.name); break;
      case "block": st.body.forEach(walkS); break;
      case "if": walkS(st.t); if (st.e) walkS(st.e); break;
      case "while": case "dowhile": walkS(st.body); break;
      case "with": walkS(st.body); break;
      case "for":
        if (st.init && "s" in (st.init as any)) walkS(st.init as Stmt);
        walkS(st.body); break;
      case "forin":
        if (st.varName) addName(st.varName);
        walkS(st.body); break;
      case "switch":
        st.cases.forEach((cl) => cl.body.forEach(walkS)); break;
    }
  };
  body.forEach(walkS);
...
let SC_DEBUG = false;
export function setScDebug(v: boolean): void { SC_DEBUG = v; }
...
const SC_KNOWN_GLOBALS = new Set<string>([
...
let SC_KNOWN_CLASSNAMES = new Set<string>();
export function resetKnownClassnames(): void { SC_KNOWN_CLASSNAMES = new Set(); }
export function addKnownClassname(name: string): void { SC_KNOWN_CLASSNAMES.add(name); }
...
let SC_KNOWN_IDS = new Set<string>();
export function resetKnownIds(): void { SC_KNOWN_IDS = new Set(); }
export function addKnownId(name: string): void { SC_KNOWN_IDS.add(name); }
const isFalse = (n: Node) => n.k === "lit" && n.v === "false";
...
const isMagicConst = (name: string) => name === "$debug" || MAGIC_FALSE.has(name) || MAGIC_TRUE.has(name);

function foldNode(n: Node): Node {
  switch (n.k) {
    case "id":
    ...
    case "logic": {
      ...
      const foldOp = (x: Node): Node => (x.k === "id" && isMagicConst(x.name)) ? x : foldNode(x);
      const l = foldOp(n.l);
      if (n.op === "&&") { if (isFalse(l)) return { k: "lit", v: "false" }; if (isTrue(l)) return foldOp(n.r); }
      else { if (isTrue(l)) return { k: "lit", v: "true" }; if (isFalse(l)) return foldOp(n.r); }
      return { k: "logic", op: n.op, l, r: foldOp(n.r) };
    }
    case "cond": {
      const c = foldNode(n.c);
      if (isFalse(c)) return foldNode(n.f);
      if (isTrue(c)) return foldNode(n.t);
    ...
    case "unary": {
      ...
      if (n.op === "!") { if (isFalse(e)) return { k: "lit", v: "true" }; if (isTrue(e)) return { k: "lit", v: "false" }; }
...
function foldStmt(s: Stmt): Stmt[] {
  ...
  const line = (s as any).line;
  if (line !== undefined) for (const r of out) if ((r as any).line === undefined) (r as any).line = line;
  const file = (s as any).file;
  if (file !== undefined) for (const r of out) if ((r as any).file === undefined) (r as any).file = file;
  const endLine = (s as any).endLine;
  if (endLine !== undefined) for (const r of out) if ((r as any).endLine === undefined) (r as any).endLine = endLine;
  const sqp = (s as any).superQuirkPredecessor;
...
function foldStmtInner(s: Stmt): Stmt[] {
  switch (s.s) {
    case "if": {
      ...
      if (isFalse(c)) return s.e ? [foldOne(s.e)] : [];
      if (isTrue(c)) return [foldOne(s.t)];
    ...
    case "funcdecl": return [{ s: "funcdecl", name: s.name, fn: foldNode(s.fn) as Extract<Node, { k: "func" }> }];
    case "for":
      return [{
        s: "for",
        init: s.init == null ? null : "s" in (s.init as any) ? foldStmt(s.init as Stmt)[0] ?? { s: "empty" } : foldNode(s.init as Node),
    ...
    case "as3class":
      return [{
        ...
        members: s.members.map((m) => m.kind === "var"
          ...
          : m.kind === "method"
            ? { ...m, fn: foldNode(m.fn) as Extract<Node, { k: "func" }> }
            : { kind: "stmt", stmt: foldStmt(m.stmt)[0] ?? { s: "empty" } }),
      }];
    default: return [s];
  }
}
function isEmptyStmt(s: Stmt): boolean {
  return s.s === "empty" || (s.s === "block" && s.body.length === 0);
}
function foldOne(s: Stmt): Stmt {
  const r = foldStmt(s);
...
function freeVarsOfNode(n: Node, declared: Set<string>, out: Set<string>): void {
  switch (n.k) {
    case "id": if (!declared.has(n.name)) out.add(n.name); break;
    ...
    case "func": for (const v of computeFree(n.params, n.body)) if (!declared.has(v)) out.add(v); break;
    case "member": freeVarsOfNode(n.o, declared, out); break; // .p is a property name
    case "index": freeVarsOfNode(n.o, declared, out); freeVarsOfNode(n.i, declared, out); break;
    case "call": freeVarsOfNode(n.c, declared, out); n.args.forEach((a) => freeVarsOfNode(a, declared, out)); break;
    case "new": freeVarsOfNode(n.c, declared, out); n.args.forEach((a) => freeVarsOfNode(a, declared, out)); break;
    case "unary": freeVarsOfNode(n.e, declared, out); break;
    case "bin": case "logic": case "assign":
      freeVarsOfNode(n.l, declared, out); freeVarsOfNode(n.r, declared, out); break;
    case "cond": freeVarsOfNode(n.c, declared, out); freeVarsOfNode(n.t, declared, out); freeVarsOfNode(n.f, declared, out); break;
    case "paren": freeVarsOfNode(n.e, declared, out); break;
    case "cast": freeVarsOfNode(n.e, declared, out); freeVarsOfNode(n.type, declared, out); break;
    case "seq": n.es.forEach((e) => freeVarsOfNode(e, declared, out)); break;
    case "array": n.els.forEach((e) => freeVarsOfNode(e, declared, out)); break;
    case "object": n.props.forEach((p) => freeVarsOfNode(p.v, declared, out)); break;
    // num, str, lit, this: no free vars
  }
}

function freeVarsOfStmts(declared: Set<string>, body: Stmt[]): Set<string> {
  const out = new Set<string>();
  const walkS = (s: Stmt): void => {
    switch (s.s) {
      case "expr": freeVarsOfNode(s.e, declared, out); break;
      case "var": s.decls.forEach((d) => { if (d.init) freeVarsOfNode(d.init, declared, out); }); break;
      case "return": if (s.e) freeVarsOfNode(s.e, declared, out); break;
      case "if": freeVarsOfNode(s.c, declared, out); walkS(s.t); if (s.e) walkS(s.e); break;
      case "block": s.body.forEach(walkS); break;
      case "while": case "dowhile": freeVarsOfNode(s.c, declared, out); walkS(s.body); break;
      case "with": freeVarsOfNode(s.c, declared, out); walkS(s.body); break;
      case "funcdecl": freeVarsOfNode(s.fn, declared, out); break;
      case "for":
        if (s.init) { if ("s" in (s.init as any)) walkS(s.init as Stmt); else freeVarsOfNode(s.init as Node, declared, out); }
        if (s.test) freeVarsOfNode(s.test, declared, out);
        if (s.upd) freeVarsOfNode(s.upd, declared, out);
        walkS(s.body); break;
      case "forin":
        if (!s.varName) freeVarsOfNode(s.lhs, declared, out);
        freeVarsOfNode(s.obj, declared, out);
        walkS(s.body); break;
      case "switch":
        freeVarsOfNode(s.disc, declared, out);
        s.cases.forEach((cl) => { if (cl.test) freeVarsOfNode(cl.test, declared, out); cl.body.forEach(walkS); });
        break;
      case "throw": freeVarsOfNode(s.e, declared, out); break;
      case "try":
        walkS(s.block); if (s.handler) walkS(s.handler); if (s.finalizer) walkS(s.finalizer);
        break;
    }
  };
  body.forEach(walkS);
...
function hasNestedFuncDecl(body: Stmt[]): boolean {
  let found = false;
  const wS = (s: Stmt): void => {
    switch (s.s) {
      case "funcdecl": found = true; break;
...
function collectVariables(body: Stmt[]): string[] {
  const order: string[] = [];
  const seen = new Set<string>();
  const add = (n: string) => { if (!seen.has(n)) { seen.add(n); order.push(n); } };
  const walkS = (st: Stmt) => {
    switch (st.s) {
      case "var": for (const d of st.decls) add(d.name); break;
      case "funcdecl": add(st.name); break;
      case "block": st.body.forEach(walkS); break;
      case "if": walkS(st.t); if (st.e) walkS(st.e); break;
      case "while": case "dowhile": case "with": walkS(st.body); break;
      case "for": if (st.init && "s" in (st.init as any)) walkS(st.init as Stmt); walkS(st.body); break;
      case "forin": if (st.varName) add(st.varName); walkS(st.body); break;
      case "switch": st.cases.forEach((cl) => cl.body.forEach(walkS)); break;
      case "try":
        walkS(st.block); if (st.param) add(st.param); if (st.handler) walkS(st.handler); if (st.finalizer) walkS(st.finalizer);
        break;
    }
  };
  body.forEach(walkS);
...
function computeFree(params: string[], body: Stmt[]): Set<string> {
  const declared = new Set<string>([...params, ...collectVariables(body), ...AVAILABLE]);
...
function collectDirectFuncs(body: Stmt[]): Extract<Node, { k: "func" }>[] {
  const out: Extract<Node, { k: "func" }>[] = [];
  const wN = (n: Node): void => {
    switch (n.k) {
      case "func": out.push(n); return; // do not descend
      case "member": wN(n.o); break;
      case "index": wN(n.o); wN(n.i); break;
      case "call": case "new": wN(n.c); n.args.forEach(wN); break;
      case "unary": wN(n.e); break;
      case "bin": case "logic": case "assign": wN(n.l); wN(n.r); break;
      case "cond": wN(n.c); wN(n.t); wN(n.f); break;
      case "paren": wN(n.e); break;
      case "cast": wN(n.e); wN(n.type); break;
      case "seq": n.es.forEach(wN); break;
      case "array": n.els.forEach(wN); break;
      case "object": n.props.forEach((p) => wN(p.v)); break;
    }
  };
  const wS = (s: Stmt): void => {
    switch (s.s) {
      case "expr": wN(s.e); break;
      case "var": s.decls.forEach((d) => { if (d.init) wN(d.init); }); break;
      case "return": if (s.e) wN(s.e); break;
      case "if": wN(s.c); wS(s.t); if (s.e) wS(s.e); break;
      case "block": s.body.forEach(wS); break;
      case "while": case "dowhile": wN(s.c); wS(s.body); break;
      case "with": wN(s.c); wS(s.body); break;
      case "funcdecl": wN(s.fn); break;
      case "for":
        if (s.init) { if ("s" in (s.init as any)) wS(s.init as Stmt); else wN(s.init as Node); }
        if (s.test) wN(s.test); if (s.upd) wN(s.upd); wS(s.body); break;
      case "forin": if (!s.varName) wN(s.lhs); wN(s.obj); wS(s.body); break;
      case "switch": wN(s.disc); s.cases.forEach((cl) => { if (cl.test) wN(cl.test); cl.body.forEach(wS); }); break;
      case "throw": wN(s.e); break;
      case "try": wS(s.block); if (s.handler) wS(s.handler); if (s.finalizer) wS(s.finalizer); break;
    }
  };
  body.forEach(wS);
...
type Scope = {
  /** Rename map applied to identifiers WITHIN the body (closed params under
   *  withThis are excluded — they are re-declared to their original name). */
  map: Map<string, string>;
  /** The emitted formal-parameter names (registers or kept names). */
  newParams: string[];
  withThis: boolean;
  /** `var name=$reg;` redeclarations to prepend inside the with-block. */
  closedRedecls: { name: string; reg: string }[];
  /** The body's free-variable set (VariableAnalyzer free). */
  free: Set<string>;
  /** VariableAnalyzer.dereferenced: the body's TOP scope (not nested closures)
   *  contains a property access (`a.b`/`a[b]`) or a call (`f(…)`). Drives the
   *  debug try/catch + `$reportException` wrapper (JavascriptGenerator:1277:
   *  `debugExceptions && (dereferenced || !free.isEmpty())`). */
  dereferenced: boolean;
  /** ALL names declared in this body (params + locals), including the closed-over
...
function computeDereferenced(body: Stmt[]): boolean {
  let found = false;
  const wN = (n: Node): void => {
    if (found) return;
    switch (n.k) {
      case "member": case "index": found = true; return;
      ...
      case "call":
        if (isSuperCallExpr(n)) { n.args.forEach(wN); return; }
        found = true; return; // also descend for sub-derefs (harmless)
      case "func": return; // nested closure: own analyzer
      case "new": n.args.forEach(wN); wN(n.c); break;
      case "unary": wN(n.e); break;
      case "bin": case "logic": case "assign": wN(n.l); wN(n.r); break;
      case "cond": wN(n.c); wN(n.t); wN(n.f); break;
      case "paren": wN(n.e); break;
...
/** AS3-class refinement of the `with(this)` decision (JavascriptGenerator:
 *  `possibleInstance = free; if (instanceprops != null) possibleInstance
 *  .retainAll(instanceprops)`). `props` is the resolvable instance-property set,
 *  passed only when the class is "complete" (no super, or all supers are AS3
 *  classes in this unit). When the instance properties are unknown the caller
 *  passes `undefined`, so `possibleInstance` is the raw free set — exactly the
 *  anonymous-class rule (the analyzer does not treat globals as non-free). */
type As3Refine = { props: Set<string> };
...
function analyzeScope(params: string[], body: Stmt[], isMethod: boolean, as3?: As3Refine, debug = false): Scope {
  ...
  for (const f of collectDirectFuncs(body))
    for (const x of computeFree(f.params, f.body)) innerFree.add(x);
  const closed = new Set<string>([...localSet].filter((n) => innerFree.has(n)));
...
class Printer {
  ...
  // compress=true is the production DHTML backend (SPACE=""); compress=false is
  // the readable/debuggable backend (SPACE=" ", ParseTreePrinter config). The
  // default is compress so every existing (production) call site is byte-identical.
  c: boolean;
  ...
  dbg = false;
  dfile = "";
  dline = 0; // current source line context for statements lacking an explicit line
  joinDepth = 0; // joinStmts recursion depth (1 = function-body StatementList)
  ...
  dbgOuterVars: Set<string> = new Set();
  ...
  outerUserName: string | null = null;
  ...
  id(name: string): string { return this.rename.get(name) ?? name; }
  ...
  lnum(line: number | null, str: string, fileOverride?: string): string {
    if (!this.dbg) return str;
  ...
  isSuperCall(n: Extract<Node, { k: "call" }>): boolean {
    const c = n.c;
    if (c.k === "super") return true;
    if (c.k === "member" && c.o.k === "super") return true;
    if (c.k === "member" && (c.p === "call" || c.p === "apply") &&
  ...
  btNotableCall(n: Node): boolean {
    if (this.btVar == null || typeof (n as { line?: number }).line !== "number" || (n as { line: number }).line <= 0) return false;
    // A `new` expression also gets noteCallSite (CodeGenerator.visitNewExpression).
    if (n.k === "new") return true;
  ...
  prec(n: Node): number {
    ...
    if ((this.btNotableCall(n) || this.btNotableId(n)) && !this.btSuppress) return 0;
    switch (n.k) {
      case "seq": return 0;
      ...
      case "call": case "new": case "member": case "index": return 17;
  ...
  wrap(child: Node, parentPrec: number, rightSide = false): string {
    const cp = this.prec(child);
    if (cp < parentPrec || (cp === parentPrec && rightSide)) return "(" + this.expr(child) + ")";
  ...
  expr(n: Node): string {
    ...
    switch (n.k) {
      ...
      case "call": {
        ...
        if (this.btVar != null && this.isSuperCall(n)) this.btSuperSeen = true;
        ...
        const btLine = (n as { line?: number }).line;
        const nextMethod = (m: string) => {
        ...
        const dispatch = (m: string) =>
          `(arguments.callee["$superclass"]${A}&&${A}arguments.callee.$superclass.prototype[${m}]` +
          `${A}||${A}${nextMethod(m)})`;
      ...
      case "assign":
        ...
        if (n.l.k === "id" && this.btNotableId(n.l)) this.btSuppress = true;
  ...
  printFunc(n: Extract<Node, { k: "func" }>): string {
    ...
    if (this.dbg) {
      ...
      if (SC_BACKTRACE) this.btSuperSeen = true;
      return out;
    }
   ...
   *  (isMethod=false). */
  printAs3Method(fn: Extract<Node, { k: "func" }>, isMethod: boolean, as3?: As3Refine): string {
    ...
    const cases = params
      .map((_, i) => {
        if (fn.defaults[i] == null) return null;
  ...
  printAs3Class(n: Extract<Stmt, { s: "as3class" }>): string {
    ...
    let refineProps: Set<string> | null;
    if (n.sup === null) { complete = true; refineProps = ownProps; }
    else {
      const sd = this.classDescriptors.get(n.sup);
      if (sd && sd.complete) { complete = true; refineProps = new Set([...ownProps, ...sd.props]); }
      else { complete = false; refineProps = null; }
    }
    ...
    const as3: As3Refine | undefined = refineProps !== null ? { props: refineProps } : undefined;

    const inst: string[] = [];
    const stat: string[] = [];
    const stmts: Stmt[] = [];
    for (const m of n.members) {
      if (m.kind === "stmt") { stmts.push(m.stmt); continue; }
    ...
    const superRef = n.mixins.length
      ...
      : n.sup;
    let args = "";
    if (statArr !== null) args = "," + statArr;
    if (superRef !== null) args = "," + superRef + args;
    else if (args.length) args = ",null" + args;
    if (instArr !== null) args = "," + instArr + args;
    else if (args.length) args = ",null" + args;
  ...
  printAs3ClassDebug(n: Extract<Stmt, { s: "as3class" }>): string {
    const ownProps = new Set<string>();
    for (const m of n.members) {
      if (m.kind === "stmt" || m.static) continue;
      ownProps.add(m.kind === "method" && m.name === n.name ? "$lzsc$initialize" : m.name);
    ...
    const file = (n as any).file !== undefined ? (n as any).file : this.dfile;
    for (const m of n.members) {
      if (m.kind === "stmt") { stmts.push(m.stmt); continue; }
      const target = m.static ? stat : inst;
      if (m.kind === "var") {
    ...
    const A = (k: number) => annoFileLine(file, k);
  ...
  joinStmts(body: Stmt[]): string {
    this.joinDepth++;
    try { return this.joinStmtsInner(body); }
    finally { this.joinDepth--; }
  }
  joinStmtsInner(body: Stmt[]): string {
    let out = "";
    let sep = "";
    ...
    for (const s of body) {
      ...
      if (this.dbg) {
        ...
        if (ruleA) line = prevEndLine;
        else if (ruleAGap) line = prevEndLine + 1;
        ...
        if (ruleA && nestedFirstSuperActive && line !== ((s as any).line ?? line)) {
          text = this.lnum((s as any).line, text, (s as any).file);
          text = this.lnum(line, text, (s as any).file);
        } else if (ruleAFired && !nestedFirstSuperActive && line !== ((s as any).line ?? line)) {
        ...
        if ((s as any).scriptVarRewrite) text = annoFileLine(null, 0) + text;
        raw = unannotateStr(text);
      }
      out += sep + text;
      sep = raw.endsWith(";") ? NLsep : ";" + NLsep;
      const sStart = (s as any).line;
      const sEnd = (s as any).endLine;
  ...
  bodyOfAt(st: Stmt, enclLine: number | undefined): string {
  ...
  stmt(st: Stmt): string {
    switch (st.s) {
      ...
      case "if": {
        ...
        if (!st.e) return cond + this.bodyOfAt(st.t, (st as any).line);
        ...
        this.pendingBlockLine = (st as any).line ?? -1;
        ...
        const elseB = st.e.s === "block"
          ? this.SP + this.bodyOfAt(st.e, (st as any).elseLine)
          : " " + this.stmt(st.e);
        return cond + thenText + this.SP + "else" + elseB;
      }
      case "while": return "while" + this.OPENP + this.expr(st.c) + this.CLOSEP + this.bodyOfAt(st.body, (st as any).line);
      case "dowhile": return "do" + this.SP + this.bodyOfAt(st.body, (st as any).line) + this.SP + "while" + this.OPENP + this.expr(st.c) + ")";
      case "with": return "with" + this.OPENP + this.expr(st.c) + this.CLOSEP + this.bodyOfAt(st.body, (st as any).line);
      case "funcdecl": {
      ...
      case "for": {
        ...
        const init = st.init == null ? "" : "s" in (st.init as any)
          ? this.forInit(st.init as Stmt) : this.expr(st.init as Node);
        return "for" + this.OPENP + init + ";" + (st.test ? this.expr(st.test) : "") + ";" +
          (st.upd ? this.expr(st.upd) : "") + this.CLOSEP + this.bodyOfAt(st.body, (st as any).line);
      }
      case "forin": {
        const head = st.varName ? "var " + this.id(st.varName) : this.expr(st.lhs);
        return "for" + this.OPENP + head + " in " + this.expr(st.obj) + this.CLOSEP + this.bodyOfAt(st.body, (st as any).line);
      }
      case "switch": {
...
function unannotateStr(s: string): string {
  if (s.indexOf(ANNOTATE_MARKER) < 0) return s;
  let out = "";
  let i = 0;
  while (i < s.length) {
...
export function collectDependencies(expr: string): { array: string; hasFree: boolean; annotation: string } {
  ...
  const fnNodes: Extract<Node, { k: "call" }>[] = [];
  const addRef = (base: Node, prop: string) => {
    const baseText = printer.expr(base);
    const text = baseText + "." + prop;
    if (!baseSeen.has(baseText)) { baseSeen.add(baseText); bases.push(jsString(baseText)); }
    if (refSeen.has(text)) return;
    refSeen.add(text);
    pairs.push(baseText + "," + jsString(prop));
  };
  const isSetAttributeOnThis = (c: Node): boolean =>
    c.k === "member" && c.o.k === "this" && c.p === "setAttribute";
  const addFn = (n: Extract<Node, { k: "call" }>) => {
    const text = printer.expr(n);
    if (fnSeen.has(text)) return;
    fnSeen.add(text);
    fnNodes.push(n);
  };
  const walk = (n: Node): void => {
    switch (n.k) {
      case "id": addRef({ k: "this" }, n.name); break;
      case "member":
        if (n.o.k === "call" && !isSetAttributeOnThis(n.o.c)) addFn(n.o);
        addRef(n.o, n.p);
        break;
      case "call":
        if (!isSetAttributeOnThis(n.c)) addFn(n);
        n.args.forEach(walk);
        break;
      case "this": case "num": case "str": case "lit": case "super": break;
      case "bin": case "logic": case "assign": walk(n.l); walk(n.r); break;
      case "unary": walk(n.e); break;
      case "cond": walk(n.c); walk(n.t); walk(n.f); break;
      case "paren": walk(n.e); break;
      case "cast": walk(n.e); walk(n.type); break;
      case "seq": n.es.forEach(walk); break;
      case "array": n.els.forEach(walk); break;
      case "object": n.props.forEach((p) => walk(p.v)); break;
      case "index": walk(n.o); walk(n.i); break;
      case "new": n.args.forEach(walk); break;
    }
  };
  walk(ast);
  ...
  for (const call of fnNodes) {
    const callee = call.c;
    if (PURE_FUNCTIONS.has(printer.expr(callee))) continue;
    let receiver: Node, method: string;
    if (callee.k === "member") { receiver = callee.o; method = callee.p; }
    else if (callee.k === "id") { receiver = { k: "this" }; method = callee.name; }
    else throw new ScUnsupported("constraint dependency on a computed call");
...
function stripVarToExpr(decls: { name: string; init: Node | null }[]): Node {
  const es: Node[] = decls.map((d) => d.init
    ? { k: "assign", op: "=", l: { k: "id", name: d.name }, r: d.init }
    : { k: "id", name: d.name });
  return es.length === 1 ? es[0] : { k: "seq", es };
}
function stripScriptVars(s: Stmt): Stmt {
  ...
  if (out !== s) {
    for (const k of ["line", "endLine", "file", "superQuirkPredecessor"]) {
      if ((s as any)[k] !== undefined && (out as any)[k] === undefined) (out as any)[k] = (s as any)[k];
    }
  }
  return out;
}
function stripScriptVarsInner(s: Stmt): Stmt {
  switch (s.s) {
    case "var": return { s: "expr", e: stripVarToExpr(s.decls), scriptVarRewrite: true } as any;
    case "for": {
      let init = s.init;
      if (init && "s" in (init as any) && (init as Stmt).s === "var")
        init = stripVarToExpr((init as Extract<Stmt, { s: "var" }>).decls);
...
export function compileScriptBody(source: string): string {
  ...
  const funcAssigns: string[] = [];
  const rest: Stmt[] = [];
  for (const s of ast) {
    if (s.s === "funcdecl") funcAssigns.push(`${s.name}=${printer.printFunc(s.fn)};`);
    else rest.push(stripScriptVars(s));
  }
  if (hasNestedFuncDecl(rest)) throw new ScUnsupported("nested function declaration in script context");
...
export function compileScriptBodyDebug(
  source: string, file: string, elementLine: number, displayCol: number
): string {
  ...
  const rest: Stmt[] = ast.filter((s) => s.s !== "funcdecl").map(stripScriptVars);
  if (hasNestedFuncDecl(rest)) throw new ScUnsupported("nested function declaration in script context");
  ...
  for (const v of hoistNames) printer.dbgOuterVars.add(v);
  ...
  // needs `;\n`.
  const joinSep = (acc: string, item: string) =>
    acc + (unannotateStr(acc).replace(/\s+$/, "").endsWith(";") ? "\n" : ";\n") + item;
  ...
  const blockNL = (b: string) => (unannotateStr(b).endsWith("}") ? "" : NL);
...
export function finalSourceLine(src: string, baseLine = 1, baseFile?: string): number {
...
export function compileProgramDebug(source: string, filename: string, baseLine: number): string[] {
  ...
  for (const s of ast) {
    ...
    const trailEmpty = s.s === "as3class" && (s as any).semi;
    ...
    if (text.startsWith("{")) { units.push(text); if (trailEmpty) units.push(";"); continue; }
    ...
    const dirLine = s.s === "as3class" && (s as any).classLine != null
      ? (s as any).classLine - 1
      : (s as any).line ?? 0;
...
export function compileStylesheetDebug(
  iifeSource: string, filename: string, elementLine: number, displayCol: number
): string[] {
  ...
  if (!iife) throw new ScUnsupported("stylesheet: no IIFE expression");
...
export function compileExpr(src: string): string {
...
export function compileExprDebug(src: string): string {
  const ast = foldNode(new Parser(lex(src)).parseExpr());
...
export function compileFunction(
  params: string[], source: string, defaults: (string | undefined)[] = [],
  isMethod = true
): string {
  ...
  const cases = params
    .map((_, i) => {
      if (defaults[i] === undefined) return null;
...
function emitDebugFunc(
  printer: Printer, params: string[], body: Stmt[], line: number, named: boolean
): string {
...
export function compileMethodDebug(
  userName: string, params: string[], source: string, filename: string, line: number
): string {
  ...
  printer.dfile = filename;
  printer.dline = line;
  if (scope.withThis) throw new ScUnsupported("debug: withThis method (grind TODO)");
...
function debugCatchBody(filename: string, line: number): string {
  return (
...
function btPrelude(dvar: string, svar: string): string {
  return "var " + dvar + " = Debug;\nvar " + svar + " = " + dvar + ".backtraceStack;";
}
function btPrefix(
  dvar: string, svar: string, avar: string,
  params: string[], newParams: string[], fn: string, line: number, isStatic: boolean
): string {
  ...
  const elems: string[] = [];
  for (let i = 0; i < params.length; i++) { elems.push(jsString(params[i])); elems.push(newParams[i]); }
...
function btSuffix(svar: string): string {
  return "if (" + svar + ") {\n" + svar + ".length--\n}";
}
...
function debugCatchBodyBacktrace(filename: string, avar: string): string {
  return (
    'if ((Error["$lzsc$isa"] ? Error.$lzsc$isa($lzsc$e) : $lzsc$e instanceof Error)' +
    ' && $lzsc$e !== lz["$lzsc$thrownError"]) {\n$reportException(' +
...
function debugCatchBodyThrows(): string {
  return (
    'if (Error["$lzsc$isa"] ? Error.$lzsc$isa($lzsc$e) : $lzsc$e instanceof Error) {\n' +
...
export function compileFunctionDebug(
  userName: string, params: string[], source: string,
  defaults: (string | undefined)[], file: string, methodLine: number, bodyBaseLine: number,
  forceWithThis = false, catchKind: "report" | "throws" = "report", isMethod = true,
  propagateName?: string | null
): string {
  ...
  const cases = params
    ...
    .filter((c): c is { i: number; assign: string } => c != null);
  ...
  const hoistAssigns = funcdecls.length
    ? funcdecls.map((d) => A(methodLine) + printer.id(d.name) + " = " +
        renderDebugFuncNode(d.fn, d.name, /*named*/ true, file, (d as any).line ?? methodLine) + ";").join("\n")
    : "";
...
export function compileBinderDebug(
  userName: string, bodySource: string, file: string, funcLine: number
): string {
  ...
  const fnStmt = ast[0];
  if (!fnStmt || fnStmt.s !== "expr" || (fnStmt.e as any).k !== "func")
    throw new ScUnsupported("binder: expected a function expression");
  const fn = foldNode((fnStmt.e as any)) as Extract<Node, { k: "func" }>;
...
function renderDebugFuncDecl(name: string, fn: Extract<Node, { k: "func" }>, file: string, funcLine: number): string {
  return renderDebugFuncNode(fn, name, /*named*/ true, file, funcLine, "report", false, undefined, undefined, undefined, name);
}

function renderDebugFuncNode(
  ...
  if (outerVars) printer.dbgOuterVars = outerVars;
  printer.dfile = file;
