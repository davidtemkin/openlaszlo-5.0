// Named color table + parseColor, ported verbatim from ViewSchema.java (4.9.0).
...
export class ColorFormatException extends Error {}

export function parseColor(str: string): number {
  if (str === "transparent") return -1;
  if (str in NAMED_COLORS) return NAMED_COLORS[str];
  ...
  if (m) {
    let v = 0;
    for (let i = 1; i <= 3; i++) {
      let s = m[i]; let c: number;
      if (s.endsWith("%")) c = (Math.trunc(parseFloat(s.slice(0, -1))) * 255) / 100;
...
export function canonicalColorHex(value: string): string {
