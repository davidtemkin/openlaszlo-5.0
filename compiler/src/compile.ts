 ...
 *  `ptype` is "ar" (app-relative) or "sr" (server-root). Dimensions are true
 *  pixels (the harness rounds resource dims to absorb the oracle's GIF artifact). */
...
export interface CompileOptions {
  /** Resolve a `resource="…"` reference to its info, or null if not found.
   *  `originId` is the id of the file the reference appeared in (for resolving
   *  relative refs against the defining file's directory). */
  resolveResource?: (ref: string, originId?: string) => ResourceInfo | null;
  /** Enumerate the frames of a multi-frame resource whose `src` names a directory
   *  or a `.swf` (→ pre-rendered `autoPng/` PNG siblings), in sorted order, or
   *  null if `src` is not such a reference. */
  resolveResourceFrames?: (ref: string, originId?: string) => ResourceInfo[] | null;
  /** App-relative master-sprite path emitted as `LzResourceLibrary.__allcss`. */
  spritePath?: string;
  /** Resolve an `<include href=…>` to its source text + a canonical id (used as
   *  the base for nested includes). null if not found. */
  resolveInclude?: (ref: string, fromId: string) => { source: string; id: string } | null;
  /** Canonical id of the root source (base for top-level includes). */
  sourceId?: string;
  /** Map a canonical origin id (absolute path) to the directive filename used in
   *  the debug `/* -*- file: X#N -*- *​/` source-location comments — relative to
   *  the component base for library files (`base/basefocusview.lzx`), the source
   *  path as given for the app file. */
  debugFileName?: (originId: string) => string;
  /** Component tag → library path (relative to the component base), for
   *  auto-inclusion of referenced-but-undefined component tags. */
  autoincludes?: Record<string, string>;
  /** Resolve a `<dataset src=…>` reference to the file's raw XML text, or null. */
  resolveDatasetSrc?: (ref: string, fromId?: string) => string | null;
  /** Resolve a `<script src=…>` reference (relative to the including file) to the
   *  referenced JS file's raw text, or null. */
  resolveScriptSrc?: (ref: string, fromId: string) => string | null;
  /** Resolve a `<font src=…>` reference (searched: app dir → components → font
   *  dir → LFC) to its server/app-relative path + ptype, or null. */
  resolveFont?: (src: string) => FontInfo | null;
  /** FORCED-DEBUG (development only): bypass the `canvas debug="true"` refusal and
   *  drive the readable/source-mapped backend (compress=false, name_$N idents,
   *  `/* -*- file: X#N -*- *​/` directives, the debugger component library, the
   *  try/catch+$reportException wrappers, $debug=true folding). Set ONLY by the
   *  harness via `LZC_DEBUG_FORCE=1`; the debug build is incomplete (not yet
   *  byte-for-byte) so this MUST NOT be set in production. */
  debug?: boolean;
  /** SOLO build: emit `canvas.__LZproxied="false"` instead of the default
   *  `"true"`. This is the ONE-byte delta between a normal (proxied) build and
   *  the oracle's SOLO build (oracle SOLO = compile property `proxied=false`,
   *  codegen site CanvasCompiler.java:193). Default (undefined / true) leaves
   *  the existing proxied behavior — and every existing gold — unchanged. */
  proxied?: boolean;
  /** Resource sprite-sheet mode. "oracle" (default) emits the oracle's
   *  `sprite:'<name>.sprite.png'` montage references for multi-frame resources
   *  (byte-for-byte parity; needs the packed PNG sheet as a runtime asset).
   *  "none" omits them, so a multi-frame resource renders from its INDIVIDUAL
   *  frame PNGs (the runtime's `LzSprite.__setFrame` frame-mode) — no montage, no
   *  compile-time image processing. Used by the Java-free `openlaszlo/` distro;
   *  diverges from the oracle deliberately. */
  sprites?: "oracle" | "none";
  /** DEBUG_BACKTRACE (`lzc -g2`): add the per-function call-stack frame
   *  instrumentation + per-call-site `$lzsc$a.lineno` notes. Byte-for-byte vs the
   *  oracle (backtrace.lzx RAW IDENTICAL). Implies `debug` (it is a debug add-on).
...
const LFC_TAG_CLASS: Record<string, string> = {
...
function classJsName(tag: string): string {
...
function parseArgs(argsStr: string): { names: string[]; defaults: (string | undefined)[] } {
  const names: string[] = [];
  ...
  for (const part of argsStr.split(",")) {
    ...
    if (eq >= 0) { names.push(name); defaults.push(p.slice(eq + 1).trim()); }
    else { names.push(name); defaults.push(undefined); }
  }
...
function elemRawText(el: XmlElem): string {
...
class LineMetrics {
  ...
  addHTML(rawtext: string, normalized: string): void {
  ...
  addSpan(str: string): void {
    ...
    if (!this.verbatim) {
      ...
      if (nl >= 0) this.last_newline_pos = nl + buflen;
    }
    this.buf += str;
  }
  addFormat(str: string): void {
    this.buf += str;
  }
  endOfLine(): void {
  ...
  paragraphBreak(): void {
    ...
    for (let i = this.buf.length - 1; i >= 0; i--) {
      ...
      if (c === "\t" || c === " ") continue;
      if (c === "\n") tn++;
      else if (this.buf.endsWith("<br/>")) { tn++; i -= "<br/>".length; }
      else break;
    }
    if (tn === 0) this.buf += "<br/><br/>";
    else if (tn === 1) this.buf += "<br/>";
    this.trim = true;
  }
}

/** Port of TextCompiler.getHTMLContent (3-arg recursive form). */
function getHTMLContentInto(el: XmlElem, lm: LineMetrics): void {
  for (const node of el.children) {
    if (node.type === "elem") {
      ...
      if (tag === "br") {
        ...
        if (elemRawText(node) !== "") lm.newline();
      } else if (tag === "p") {
        lm.paragraphBreak();
        ...
        lm.addFormat("<" + tag);
        for (const an of node.attrOrder) lm.addFormat(" " + an + '="' + node.attrs[an] + '"');
        lm.addFormat(">");
        getHTMLContentInto(node, lm);
        lm.addFormat("</" + tag + ">");
      }
      ...
      if (lm.verbatim) lm.addSpan(raw);
      else if (raw.length > 0) lm.addHTML(raw, normalizeTextContent(raw));
    }
  }
}
function getHTMLContent(el: XmlElem): string {
...
function getInputText(el: XmlElem): string {
  let text = "";
  for (const node of el.children) {
    if (node.type === "elem") {
      if (node.name === "p" || node.name === "br") text += "\n";
      else if (node.name === "pre") text += elemRawText(node);
    } else {
...
function isLocalDataset(el: XmlElem): boolean {
  const type = el.attrs["type"];
  if (type === "soap" || type === "http") return false;
  const src = el.attrs["src"];
  if (src != null && (/^https?:/.test(src) || /^\s*\$(\w*)\s*\{[\s\S]*\}\s*$/.test(src))) return false;
  return true;
}
...
function isLiteralDatasetEl(el: XmlElem): boolean {
  if (el.name !== "dataset") return false;
  const dtype = el.attrs["type"];
  const dsrc = el.attrs["src"];
  if (dtype === "soap" || dtype === "http") return false;
  if (dsrc != null && (/^https?:/.test(dsrc) || /^\s*\$(\w*)\s*\{[\s\S]*\}\s*$/.test(dsrc))) return false;
  if (el.attrs["datafromchild"] === "true") return false;
  return true;
}
...
function datasetArgs(el: XmlElem, globals: string[], globalOrigins: string[], opts: CompileOptions):
  { name: string; content: string; trim: boolean; nsprefix: boolean } {
  const name = el.attrs["name"];
  if (!name) throw new Unsupported(`<dataset> without name`);
  ...
  if (src != null) {
    ...
    if (text == null) throw new Unsupported(`unresolved dataset src: ${src}`);
  ...
  if (trim) throw new Unsupported(`dataset trimwhitespace`); // TODO: trimWhitespace pass
  const dataEl: XmlElem = { type: "elem", name: "data", attrs: {}, attrOrder: [], children };
...
function compileDataset(el: XmlElem, globals: string[], globalOrigins: string[], opts: CompileOptions): string {
...
function compileDatasetDebug(
  el: XmlElem, globals: string[], globalOrigins: string[], opts: CompileOptions,
  dir: [string, number]
): string[] {
  const { name, content, trim, nsprefix } = datasetArgs(el, globals, globalOrigins, opts);
...
function serializeXmlRaw(el: XmlElem): string {
  let out = "<" + el.name;
  for (const a of el.attrOrder) out += ` ${a}="${escapeXmlAttr(el.attrs[a])}"`;
  if (el.children.length === 0) return out + " />";
  out += ">";
  for (const c of el.children) {
    if (c.type === "elem") out += serializeXmlRaw(c);
    else if (c.comment) out += `<!--${c.value}-->`;
    else out += escapeXmlText(c.value);
  }
  return out + `</${el.name}>`;
}
...
function parseConstraint(raw: string): { when: string; expr: string } | null {
...
function attrConstraint(
  raw: string | null,
  whenAttr: string | undefined
): { when: string; expr: string; literal: boolean } | null {
  if (raw == null) return null;
  const c = parseConstraint(raw);
  if (c) return { ...c, literal: false };
  ...
  if (whenAttr != null) return { when: whenAttr === "always" ? "" : whenAttr, expr: raw, literal: true };
  return null;
}
...
const DEPS_INNER = (deps: string): string =>
  `try{\nreturn ${deps}\n}\ncatch($lzsc$e){\n` +
  'if(Error["$lzsc$isa"]?Error.$lzsc$isa($lzsc$e):$lzsc$e instanceof Error){\n' +
  "lz.$lzsc$thrownError=$lzsc$e\n};throw $lzsc$e\n}";
const depsMethod = (deps: string, hasFree: boolean): string =>
  hasFree
    ? `function(){\nwith(this){\n${DEPS_INNER(deps)}}}`
    : `function(){\n${DEPS_INNER(deps)}}`;

interface Constraint {
  entries: string[]; // instance-prop method entries (setter [+ deps for `always`])
  initExpr: string; // `new Lz{Always,Once}Expr(…)` init value
  // Debug build only: the body SOURCE + source line of the LAST code member this
  // constraint emits (the deps for `always`, the setter for `once`) — feeds the
  // member-rich anon-class constructor source-line simulation (see emitNode).
  lastBody?: string;
  lastSrcLine?: number;
}
...
const srcDirective = (file: string, line: number) => `\n#file ${file}\n#line ${line}\n`;
const END_SRC_DIRECTIVE = "\n#file \n";
const attrSrc = (file: string, line: number, value: string) =>
  "#beginAttribute" + srcDirective(file, line) + value + END_SRC_DIRECTIVE + "#endAttribute";
...
function compileConstraintDebug(
  name: string, exprType: string, expr: string, when: string,
  mGen: SymbolGenerator, file: string, srcLine: number
): Constraint {
  ...
  const setterBody =
    `var $lzc$newvalue = ${attrSrc(file, srcLine, expr)};\n` +
...
function colorValue(
  name: string, raw: string, declared: string, mGen: SymbolGenerator, file?: string, srcLine?: number
): { plain: string } | { entries: string[]; init: string; cc: Constraint } {
  try {
    return { plain: `LzColorUtils.convertColor(${jsString(canonicalColorHex(raw))})` };
  } catch (e) {
    if (!(e instanceof ColorFormatException)) throw e;
...
function styleConstraintExpr(name: string, exprType: string, value: string, fallback?: string): string {
  const v = value.trim();
  if (!/^(?:'\S*'|"\S*")$/.test(v))
...
function aliasType(t: string): string {
...
function btNoteConstraintInit(initExpr: string, line: number, file: string): string {
  if (!COMPILE_BACKTRACE) return initExpr;
  const m = /^new ([A-Za-z_$][\w$.]*)\(([\s\S]*)\)$/.exec(initExpr);
  if (!m) return initExpr;
...
function btNoteColorInit(plain: string, line: number): string {
  if (!COMPILE_BACKTRACE) return plain;
...
const LZX_TYPE: Record<string, AttrType> = {
  number: "number",
  string: "string",
  boolean: "boolean",
  color: "color",
...
function asNumber(raw: string): Typed {
  const s = raw.trim();
  if (/^-?\d+$/.test(s)) return { kind: "number", v: parseInt(s, 10) };
  if (/^-?\d*\.\d+$/.test(s)) return { kind: "number", v: parseFloat(s) };
...
function compileAttr(
  tag: string, name: string, raw: string, inCanvas: boolean,
  typeOf: (t: string, n: string) => AttrType = attrType
): string {
  ...
  if (name === FONTSTYLE_ATTRIBUTE) raw = normalizeStyleString(raw);
...
function normalizeStyleString(style: string): string {
  ...
  for (const tok of style.trim().split(/\s+/)) {
    if (tok === "") continue;
    else if (tok === "bold") bits |= 1;
    else if (tok === "italic") bits |= 2;
    else if (tok === "plain") bits |= 0;
    else if (tok === "bolditalic") bits |= 3;
    else throw new Unsupported(`unknown fontstyle token: ${tok}`);
  }
  return ["plain", "bold", "italic", "bolditalic"][bits];
}

/** Compile a raw value given an explicit attribute type. */
function compileTypedValue(t: AttrType, raw: string, inCanvas: boolean): string {
  switch (t) {
    case "color":
      if (inCanvas) return emitTyped({ kind: "number", v: parseColor(raw) });
      try {
    ...
    case "number":
      ...
      if (raw.trim().endsWith("%")) return emitTyped({ kind: "string", v: raw });
...
function compileCss(raw: string): string {
  ...
  for (const decl of text.split(";")) {
    ...
    if (ci < 0) {
      // Bare property → true.
      const key = d.trim();
      if (!/^[A-Za-z][A-Za-z0-9-]*$/.test(key)) throw new Unsupported(`css property: ${key}`);
      props[key] = "true";
      continue;
    }
...
function compileCssTerm(v: string): string {
  ...
  if (/^[A-Za-z][A-Za-z0-9-]*$/.test(v)) return emitTyped({ kind: "string", v });
  throw new Unsupported(`css value: ${v}`);
}

/** Resolve an LZX tag to its runtime JS class reference. */
type Resolve = (tag: string) => string;
...
function buildSpec(el: XmlElem, resolve: Resolve, classDepth: number | null = null): Spec {
  const cls = resolve(el.name);
  const attrs: Record<string, string> = {};
  if (classDepth != null) attrs["$classrootdepth"] = String(classDepth);
  ...
  for (const c of el.children) {
    if (c.type !== "elem") { textParts.push(c.value); continue; }
    if (c.name === "doc") continue; // documentation
    if (isHTMLElement(c.name)) continue; // folded into `text` by getHTMLContent
  ...
  if (el.name === "text" && !("text" in attrs)) {
    ...
    if (t.length !== 0) attrs["text"] = jsString(t);
...
function emitSpec(s: Spec): string {
  const obj: Record<string, string> = { class: s.cls };
  if (Object.keys(s.attrs).length > 0) obj.attrs = emitObject(s.attrs);
...
function childViews(el: XmlElem): XmlElem[] {
  return el.children.filter(
    (c): c is XmlElem =>
      c.type === "elem" && c.name !== "doc" && c.name !== "datapath" &&
...
function buildChildrenProp(
  ...
  resolve: Resolve,
  superTag: string,
  classDepth: number | null,
  classDefChildCount: (tag: string) => number
): string | null {
  const inherits = classDefChildCount(superTag) > 0;
  if (childEls.length === 0 && !inherits) return null;
...
function ent(name: string, value: string): string { return jsString(name) + (COMPILE_DEBUG ? ", " : ",") + value; }
/** Emit an object literal honoring the build mode: spaced `{k: v, …}` (sorted)
 *  for the debug backend, tight `{k:v,…}` for production. */
function emitObj(o: Record<string, string>): string { return COMPILE_DEBUG ? emitObjectSpaced(o) : emitObject(o); }
function voidSlot(name: string): string { return jsString(name) + (COMPILE_DEBUG ? ", void 0" : ",void 0"); }
...
function adjustRelativePathJ(p: string, source: string, dest: string): string {
  ...
  while (sd.length && dd.length && sd[0] === dd[0]) { sd.shift(); dd.shift(); }
  const comps: string[] = [];
  for (let i = 0; i < sd.length; i++) comps.push("..");
  for (const d of dd) comps.push(d);
  comps.push(p);
  return comps.join("/");
}
...
function urlSchemeSource(raw: string, srcDir: string, destDir: string): string {
  ...
  if (rest.startsWith("//")) {
    ...
    if (authority.length > 0) return raw; // non-empty host → keep verbatim
  ...
  if (path.startsWith("/")) return raw; // absolute path → keep verbatim
  ...
  if (srcDir && destDir && srcDir !== destDir) path = adjustRelativePathJ(path, srcDir, destDir);
...
function isVoidSlot(e: string): boolean { return e.endsWith(",void 0") || e.endsWith(", void 0"); }
function splitEntry(e: string): { name: string; value: string } | null {
...
function routeStateMethods(instEntries: string[], inits: Record<string, string>): string[] {
  return instEntries.map((e) => {
    const m = splitEntry(e);
    if (!m || isVoidSlot(e)) return e;
    inits[m.name] = m.value;
    return voidSlot(m.name);
  });
}

/** Emit a class definition. `instEntries` are formatted `"name",value` instance
 *  properties (attribute slots + methods), in order; the default constructor is
 *  always appended. With default attributes a mergeAttributes block is added;
 *  otherwise it is a bare `Class.make(…);` statement. */
/** Debug source context for a class: the directive filename + the class-open
 *  line (endLine−1), the body line (endLine, used by mergeAttributes), and the
 *  synthetic constructor's source line. `memberRich` selects the constructor
 *  rendering (plain/file="" vs member-less directive form). */
interface DebugClassCtx { file: string; classLine: number; bodyLine: number; ctorLine: number; memberRich: boolean; }

function emitClassBlock(
  name: string,
  superJs: string,
  instEntries: string[],
  defaultAttrs: Record<string, string>,
  childrenJs: string | null,
  classAllocEntries: string[] = [],
  datapathSlot = false,
  dbg?: DebugClassCtx
): string {
  ...
  if (dbg) {
    ...
    if (datapathSlot) tailEntries.push(`"$datapath", void 0`);
    ...
    if (Object.keys(defaultAttrs).length === 0) return make;
...
function emitAnonClass(
  lzcName: string, superJs: string, superTag: string, instEntries: string[], childrenJs: string | null
): string {
...
function emitAnonClassDebug(
  lzcName: string, superJs: string, superTag: string, instEntries: string[], childrenJs: string | null, node: BuiltNode
): string {
  const file = node.el ? debugFile(node.el) : "";
  const classLine = (node.el?.endLine ?? node.el?.line ?? 1) - 1; // start-tag `>` line − 1
  if (node.lastMemberBody === undefined) throw new Unsupported(`debug anon class without a tracked code member`);
  ...
  let lastMethodIdx = -1;
  for (let i = 0; i < instEntries.length; i++) if (!isVoidSlot(instEntries[i])) lastMethodIdx = i;
  ...
  if (firstChildBinder) firstChildBinder.funcLine = classLine + 3;
  const dn = jsString(`<anonymous extends='${superTag}'>`);
  const childrenPart = childrenJs ? `"children", ${childrenJs}, ` : "";
...
/** A built instance node: its method entries (for an anon class, if any), its
 *  non-method attrs, and its built view-children. `buildNode` allocates ALL
 *  method `$m` gensyms (pre-order, matching the oracle's model-build pass);
 *  `emitNode` then allocates anon-class names (pre-order) and emits classes
 *  child-before-parent (matching the asMap/emit pass). */
interface BuiltNode {
  superTag: string;
  ...
  children: BuiltNode[];
  /** Named-child reference slots (`<name>:void 0`) declared on this node. */
  slotNames: string[];
  /** Regular named-child slots already interleaved into `methodEntries` in
   *  document order (so they appear among methods in the anon class, not after).
   *  State-hoisted names (in `slotNames` but not here) are appended instead. */
  inlineSlots: string[];
  /** Pure-value `<attribute>` decl slots (non-inherited, no setter/constraint):
   *  do not force an anon class; emitted as instance attrs or class decls. */
  attrSlots: string[];
  /** A `<datapath>` child → the `$datapath` instance attr (+ `datapath`
   *  overridden to `LzNode._ignoreAttribute`). Not in `children`. */
  datapath?: BuiltNode;
  /** For `<script>` elements: the compiled `function(){…}` body. */
  script?: string;
  /** Debug build (reg/trailer cross-unit line-state, S36): true iff this node's
   *  subtree emits at least one SOURCE-literal VALUE attribute — one the oracle
   *  wraps in `#beginAttribute …#file <app>…#file <reset>#endAttribute`. The
   *  trailing empty `#file ` resets the JavaCC lexer's `Token.currentPathname` to
   *  "" and is never restored, so when the LAST top-level instance's subtree ends
   *  on such a reset the tag-map registrations + trailer inherit generated context
   *  (NO `/* file *​/` directives — "Pattern A"). Auto-injected attrs (e.g.
   *  `clickable: true` from an onclick handler) are NOT source attrs and render
   *  bare, so they do NOT count. Constraints/binders/$delegates/void-slots emit no
   *  `#file` either. `subtreeHasScriptOrContent` flags a `<script>` instance or
   *  folded text content in the subtree — those use `#beginContent` (NOT
   *  `#beginAttribute`) whose reset the oracle silently re-establishes (the
   *  color-$3/databinding-$10 anomaly), so a content-ending instance stays Pattern
   *  B; the guard excludes it. */
  subtreeHasLiteralAttr?: boolean;
  subtreeHasScriptOrContent?: boolean;
  /** Debug build (childOnlyRich synthetic-ctor line): the source line of the LAST
   *  `#line` directive the children static block emits — the start-tag-close line
   *  (el.endLine) of the LAST element (in DFS document order) in this subtree that
   *  carries a SOURCE-literal value attribute. The synthetic constructor (loc=null)
   *  inherits the generated context from THAT line, so the ctor's `$reportException`
   *  line = lastLiteralLine + 4 + trailingVoidSlots. This is the deep generalization
   *  of the old `lastOwnChild.closeLine + 4`: when the last child's last literal attr
   *  is a GRANDCHILD's (datepickercombobox: outer view closes @55 but the deepest
   *  literal — basebutton's styleable — is @54), the grandchild line is the anchor.
   *  undefined when the subtree emits no literal attr. */
  subtreeLastLiteralLine?: number;
  /** Debug build (S36): true iff THIS node is a TOP-LEVEL instance with an `id=` or
   *  `name=` attribute (→ a global `var id/name` + `bind_id`/`bind_name` binder). A
   *  top-level id/name causes the oracle to re-establish `#file <app>` AFTER the
   *  instance (a hidden re-set, like the color-$3 `<script>` anomaly — the
   *  named-instance registration / global-decl path), so the tag-map then sees the
   *  REAL file context → Pattern B, even if the instance's own attrs ended on a
   *  `#file ` reset. (class-inheritance-$26 `<container name="thetop">` and a
   *  top-level `<view id="…">` are both Pattern B; the unnamed `<container>` of $30
   *  is Pattern A.) Guards the reg discriminator: an id/name'd last instance stays B. */
  topLevelHasIdOrName?: boolean;
  /** True when this node's class transitively extends `<state>`: its methods
   *  install INLINE in the instance attrs (canHaveMethods=false), with no anon
   *  subclass (ClassModel.java's `!isstate` guard, instance side). */
  isState?: boolean;
  /** True when this node instantiates an `<interface>`: the asMap uses `tag:
   *  "name"` deferred indirection instead of `"class":<ref>` (NodeModel.asMap). */
  isInterface?: boolean;
  /** Source element (for the debug build's anon-class source directive +
   *  synthetic-constructor line). */
  el?: XmlElem;
  /** The last code-generating member's close line + whether it is a handler
   *  (for the debug anon-class constructor source line). -1 if member-less. */
  lastMemberClose?: number;
  lastMemberHandler?: boolean;
  /** Debug build: the body SOURCE + source line of the LAST code member emitted
   *  on this node (constraint deps/setter, method, handler, setter). Feeds the
   *  member-rich anon-class constructor source-line simulation (emitNode). */
  lastMemberBody?: string;
  lastMemberSrcLine?: number;
  /** Debug build: the bind_id binder spec for this node's `id` (if any), so an
   *  enclosing anon class can re-point its funcLine to ctorLine − 4 when this node
   *  is the anon class's FIRST child (a class-def-first Pattern-B binder). */
  idBinderSpec?: BinderSpec;
}

/** Per-compile resolvers/state threaded through node building. */
interface Ctx {
  resolve: Resolve;
  resolveConstraintType: (tag: string, attr: string, parentTag?: string) => string;
  valueTypeOf: (tag: string, attr: string) => AttrType;
  isInherited: (superTag: string, attr: string) => boolean;
  mGen: SymbolGenerator;
  globals: string[];
  /** Parallel to `globals`: the source-file origin of each global, used to order
   *  the declarations per the oracle's getLibraries/computeDeclarations passes. */
  globalOrigins: string[];
  registerResource: (ref: string, originId?: string) => string;
  isStateClass: (tag: string) => boolean;
  /** The `when=` of the first inherited `<attribute>` declaration for `attr`
   *  walking the user-class chain (undefined if none) — drives the inherited
   *  `when="once|always"` style-system propagation on instances. */
  inheritedWhen: (className: string, attr: string) => string | undefined;
  /** Whether the tag's class has a `text` instance attribute (addText trigger). */
  hasTextContent: (tag: string) => boolean;
  /** Whether the tag's class transitively extends `<inputtext>`. */
  isInputTextTag: (tag: string) => boolean;
  classes: Map<string, ClassDef>;
  /** Names declared via `<interface>` (emit nothing; instances use `tag:`). */
...
function collectNamedChildren(
  ...
  if (superTag) {
    ...
    if (def) names.push(...collectNamedChildren(childViews(def.el), def.superTag, ctx));
  }
  return names;
}
...
function percentConstraintExpr(schemaType: string | null, name: string, raw: string): string | null {
  if (schemaType !== "size" && schemaType !== "numberExpression") return null;
  const v = raw.trim();
...
function buildNode(el: XmlElem, ctx: Ctx, topLevel: boolean, classDepth: number | null, parentTag?: string): BuiltNode {
  ...
  const delegateEvents = new Set<string>();
  const attrs: Record<string, string> = {};
  ...
  let lastMemberBody: string | undefined;
  let lastMemberSrcLine: number | undefined;
  ...
  let idBinderSpec: BinderSpec | undefined;
  const noteCodeMember = (body: string | undefined, srcLine: number | undefined) => {
    if (body !== undefined) { lastMemberBody = body; lastMemberSrcLine = srcLine; }
  };
  ...
  let pendingNameGlobal: string | undefined;
  let pendingNameBinderRaw: string | undefined;
  // <script> compiles to an `lz.script` instance carrying a `script` function.
  if (el.name === "script") {
    if (el.attrs["when"] === "immediate") throw new Unsupported(`<script when="immediate">`);
    ...
    if (src) {
      ...
      if (text == null) throw new Unsupported(`<script src="${src}"> not found`);
      body = `#file ${src}\n#line 1\n` + text;
    }
  ...
  if (el.name === "dataset") {
    ...
    if (literal) {
      datasetLiteral = true;
      let dchildren: XmlNode[];
      if (dsrc != null) {
        const text = DATASET_SRC?.(dsrc, el.origin);
        if (text == null) throw new Unsupported(`unresolved dataset src: ${dsrc}`);
        dchildren = [parseXml(text, { keepComments: true })];
      } else {
        dchildren = el.children;
      }
      if (el.attrs["trimwhitespace"] === "true") throw new Unsupported(`dataset trimwhitespace`);
      const dataEl: XmlElem = { type: "elem", name: "data", attrs: {}, attrOrder: [], children: dchildren };
  ...
  for (const name of el.attrOrder) {
    ...
    if (name === "resource" && !parseConstraint(raw) && /^(?:https?|ftp|file|soap):/.test(raw)) {
      ...
      globals.push(raw); addKnownId(raw);
      globalOrigins.push(el.origin ?? "");
      ...
      if (COMPILE_DEBUG) idBinderSpec = lastBinderSpec();
      ...
      if (!con && raw.trim().endsWith("%")) {
        try { ctype = resolveConstraintType(superTag, name, parentTag); } catch { ctype = null; }
        const pexpr = percentConstraintExpr(ctype, name, raw);
        if (pexpr) con = { when: "", expr: pexpr };
      }
      ...
      if (!con && !isEventAttr(name)) {
        ...
        if (iw === "once" || iw === "always") {
          ...
          if (ac) { con = { when: ac.when, expr: ac.expr }; literal = ac.literal; }
        }
      }
      if (con) {
        if (con.when === "style") {
          ...
          if (con.when !== "" && con.when !== "once" && con.when !== "path") throw new Unsupported(`$${con.when}{} constraint`);
        ...
        if ("plain" in cv) { attrs[name] = cv.plain + (COMPILE_DEBUG ? litReset(el.endLine ?? el.line ?? 0) : ""); hasLiteralAttr = true; } // a plain color literal wraps in #beginAttribute
        else { methodEntries.push(...cv.entries); attrs[name] = cv.init; noteCodeMember(cv.cc.lastBody, cv.cc.lastSrcLine); }
      } else {
  ...
  if (pendingNameGlobal !== undefined && pendingNameGlobal !== el.attrs["id"]) {
    globals.push(pendingNameGlobal); addKnownId(pendingNameGlobal); globalOrigins.push(el.origin ?? "");
  }
  ...
  let datapathNode: BuiltNode | undefined;
  let textParts = "";
  ...
  for (const c of datasetLiteral ? [] : el.children) {
    if (c.type === "text") { textParts += c.value; continue; }
    if (c.name === "doc") continue;
    ...
    if (isHTMLElement(c.name)) continue;
    if (c.name === "handler") { const h = compileHandler(c, mGen, methodEntries, delegateList, delegateEvents); noteCodeMember(h.body, h.srcLine); }
    else if (c.name === "method") { methodEntries.push(compileMethod(c)); noteCodeMember(c.children.map((n) => (n.type === "text" ? n.value : "")).join("") + "\n#endContent", c.line ?? 0); }
      ...
      if (!an) throw new Unsupported(`<attribute> without name`);
      ...
      if (con && con.when === "immediately") {
        ...
        if (!isInherited(superTag, an)) attrSlots.push(an);
      ...
      if (!en) throw new Unsupported(`<event> without name`);
      if (!isInherited(superTag, en)) methodEntries.push(voidSlot(en));
      attrs[en] = "LzDeclaredEvent";
    } else if (c.name === "setter") {
      const sn = c.attrs["name"];
      if (!sn) throw new Unsupported(`<setter> without name`);
      ...
      if (datapathNode) throw new Unsupported(`multiple <datapath> children`);
      datapathNode = buildNode(c, ctx, false, null, superTag);
      if (datapathNode.datapath) throw new Unsupported(`nested <datapath>`);
  ...
  if (!ctx.isStateClass(superTag)) {
    for (const c of childViews(el)) {
      if (!ctx.isStateClass(c.name)) continue;
      for (const h of collectNamedChildren(childViews(c), c.name, ctx))
        if (!slotNames.includes(h) && !(h in attrs)) slotNames.push(h);
    }
  }
  ...
  if (ctx.hasTextContent(superTag) && !("text" in attrs)) {
    ...
    if (text.length !== 0) { attrs["text"] = jsString(text); }
  ...
  let subtreeLastLiteralLine: number | undefined =
    hasLiteralAttr ? (el.endLine ?? el.line ?? undefined) : undefined;
  if (!becomesAnonClass)
    for (const c of children)
      if (c.subtreeLastLiteralLine != null) subtreeLastLiteralLine = c.subtreeLastLiteralLine;
...
function emitNode(
  node: BuiltNode,
  resolve: Resolve,
  inheritsChildren: (tag: string) => boolean,
  mGen: SymbolGenerator,
  compileClass: (name: string) => string
): { defs: string; map: string } {
  if (node.script != null)
    return {
      defs: "",
      map: COMPILE_DEBUG
  ...
  if (!("clickable" in attrs)) {
    ...
    if (attrs["cursor"] === "true" || mouseAttr || mouseDelegate) attrs["clickable"] = "true";
  }
  if (node.delegateList.length > 0) attrs["$delegates"] = "[" + node.delegateList.join(COMPILE_DEBUG ? ", " : ",") + "]";
  ...
  if (node.isState) {
    for (const e of node.methodEntries) {
      ...
      const m = splitEntry(e);
      if (!m) throw new Unsupported(`state instance with declaration slot`);
  ...
  if (className) {
    ...
    let childrenJs: string | null = null;
    ...
    const spec: Record<string, string> = { class: className };
    if (Object.keys(attrs).length > 0) spec.attrs = emitObj(attrs);
    // Order: anon's super (compiled first), then its children's defs, then its own.
    if (DEBUG_STMTS) { pushDebug(classDef); return { defs: "", map: emitObj(spec) }; }
  ...
  const spec: Record<string, string> = node.isInterface
  ...
  if (COMPILE_DEBUG && Object.keys(attrs).length === 0) {
    ...
    if (fb) fb.funcLine = node.el?.endLine ?? node.el?.line ?? fb.funcLine;
  }
  if (childMaps.length > 0) spec.children = "[" + childMaps.join(COMPILE_DEBUG ? ", " : ",") + "]";
  if (DEBUG_STMTS) return { defs: "", map: emitObj(spec) };
...
function idBinder(symbol: string, setId: boolean): string {
...
function idBinderBodySource(symbol: string, setId: boolean): string {
  const q = jsString(symbol);
  return (
    `#pragma "userFunctionName=bind #${symbol}"\n` +
    `if ($lzc$bind) {\n` +
    `  if ($debug) {\n` +
...
function idBinderDebug(symbol: string, setId: boolean, file: string, elemLine: number): string {
...
function binderLineSpan(setId: boolean): number {
...
function emitHandler(
  ...
  body: string | null,
  argsAttr: string | undefined,
  reference: string | undefined,
  methodAttr: string | undefined,
  mGen: SymbolGenerator,
  methodEntries: string[],
  delegateList: string[],
  delegateEvents: Set<string>,
  ...
  if (hasBody) return { body: body! + "\n#endContent\n", srcLine: bLine };
  if (reference != null) return { body: `return (${attrSrc(file, hLine, reference)});`, srcLine: hLine };
  return {};
}
...
function compileHandler(
  ...
  return emitHandler(c.attrs["name"], body, c.attrs["args"], c.attrs["reference"],
    c.attrs["method"], mGen, methodEntries, delegateList, delegateEvents, c);
}
...
function bodyLineOf(c: XmlElem): number {
  for (const n of c.children) if (n.type === "text") return n.line ?? c.line ?? 0;
  return c.line ?? 0;
}

/** Compile a `<method>` child element to its `"name",function(…){…}` entry. */
function compileMethod(c: XmlElem): string {
  const mn = c.attrs["name"];
  if (!mn) throw new Unsupported(`<method> without name`);
  const { names, defaults } = parseArgs(c.attrs["args"] || "");
  const body = c.children.map((n) => (n.type === "text" ? n.value : "")).join("");
...
const COMPILE_CONSTANTS: Record<string, boolean | string> = {
  $runtime: "dhtml",
...
function evaluateSwitch(el: XmlElem): XmlElem[] {
  for (const c of el.children)
    if (c.type === "elem" && c.name !== "when" && c.name !== "unless" && c.name !== "otherwise")
      throw new Unsupported(`<switch> clause <${c.name}>`);
  let selected: XmlElem | null = null;
  for (const c of el.children)
    if (c.type === "elem" && c.name === "when" && evalSwitchCondition(c)) { selected = c; break; }
  for (const c of el.children)
    if (c.type === "elem" && c.name === "unless" && !evalSwitchCondition(c)) { selected = c; break; }
  if (!selected)
    for (const c of el.children)
      if (c.type === "elem" && c.name === "otherwise") { selected = c; break; }
  return selected ? selected.children.filter((n): n is XmlElem => n.type === "elem") : [];
}
...
function expandIncludes(
  ...
  if (!ORIGIN_RANK.has(currentId)) ORIGIN_RANK.set(currentId, ORIGIN_RANK_NEXT++);
  const out: XmlNode[] = [];
  const processChild = (c: XmlNode) => {
    if (c.type === "elem" && c.name === "include") {
      const href = c.attrs["href"];
      if (!href) throw new Unsupported(`<include> without href`);
      ...
      if (c.attrs["type"] === "text") {
        ...
        if (!inc) throw new Unsupported(`unresolved text include: ${href}`);
      ...
      const inc = opts.resolveInclude?.(href, currentId);
      if (!inc) throw new Unsupported(`unresolved include: ${href}`);
      let incRoot: XmlElem;
      try {
        incRoot = parseXml(inc.source);
      } catch (e) {
        throw new Unsupported(`include ${href} parse: ${(e as Error).message}`);
      }
      if (incRoot.name === "library") {
        ...
        for (const ic of incRoot.children) if (ic.type === "elem") out.push(ic);
      } else {
      ...
      for (const sel of evaluateSwitch(c)) processChild(sel);
    } else {
      if (c.type === "elem") expandIncludes(c, currentId, opts, seen, recordOrigins);
      out.push(c);
    }
  };
  for (const c of el.children) processChild(c);
  el.children = out;
}
...
function collectLayoutRef(el: XmlElem, referenced: Set<string>): void {
...
function collectTags(el: XmlElem, defined: Set<string>, referenced: Set<string>): void {
  for (const c of el.children) {
...
function expandAutoincludes(root: XmlElem, opts: CompileOptions, _seen: Set<string>): number {
  ...
  const definedOrigin = new Map<string, string | undefined>(); // tag -> defining file
  for (const c of root.children)
    if (c.type === "elem" && (c.name === "class" || c.name === "interface" || c.name === "mixin")) {
      const n = c.attrs["name"];
      if (n && !definedOrigin.has(n)) definedOrigin.set(n, c.origin ?? undefined);
    }

  // Map each app-referenced autoincludable tag (not already defined) to its lib.
  const libs = new Map<string, { source: string; id: string }>(); // id -> lib
  for (const tag of referenced) {
    if (!auto[tag]) continue;
    ...
    if (definedOrigin.has(tag) && definedOrigin.get(tag) !== inc.id) continue;
    libs.set(inc.id, inc);
  }
  ...
  for (const id of [...libs.keys()].sort()) {
    ...
    for (const c of libRoot.children) if (c.type === "elem") prefix.push(c);
  }
  ...
  const canvasContent = root.children.filter(
    (c) => !(c.type === "elem" && c.origin != null && seen.has(c.origin))
  );
  ...
  for (const id of seen) _seen.add(id);
...
function spliceDebuggerLibrary(root: XmlElem, opts: CompileOptions, seen: Set<string>, at: number): void {
  const inc = opts.resolveInclude?.("debugger/library.lzx", opts.sourceId ?? "");
  if (!inc) throw new Unsupported(`debug build: cannot resolve debugger/library.lzx`);
  if (seen.has(inc.id)) return;
  seen.add(inc.id);
  const libRoot = parseXml(inc.source);
  expandIncludes(libRoot, inc.id, opts, seen, AUTO_ORIGINS);
  const children = libRoot.children.filter((c): c is XmlElem => c.type === "elem");
...
let SCRIPT_SRC: ((ref: string, fromId: string) => string | null) | null = null;
...
let DATASET_SRC: ((ref: string, fromId?: string) => string | null) | null = null;
...
let DEBUG_FILE: (id: string) => string = (id) => id;
let DEBUG_SOURCE_ID = "";
function debugFile(el: XmlElem): string {
...
function debugMergeAttributes(file: string, classLine: number, bodyLine: number, classNameJs: string, objSpaced: string, mergeLine?: number, noteLine?: number): string {
...
function debugCatchBody(file: string, line: number): string {
  return (
    'if ((Error["$lzsc$isa"] ? Error.$lzsc$isa($lzsc$e) : $lzsc$e instanceof Error)' +
    ' && $lzsc$e !== lz["$lzsc$thrownError"]) {\n$reportException(' +
...
let DEBUG_STMTS: string[] | null = null;
function pushDebug(stmt: string): void { if (DEBUG_STMTS) DEBUG_STMTS.push(stmt); }
...
function instanceTrailingFile(built: BuiltNode | undefined, file: string): string {
...
let AUTO_ORIGINS = new Set<string>();
...
let LIBRARY_ORIGINS = new Set<string>();
...
let ORIGIN_RANK = new Map<string, number>();
...
function orderGlobals(globals: string[], origins: string[], sourceId: string): string[] {
  const cat = (o: string): number => (AUTO_ORIGINS.has(o) ? 0 : LIBRARY_ORIGINS.has(o) ? 2 : 1);
  ...
  const rank = (o: string): number => ORIGIN_RANK.get(o) ?? 0;
  return globals
...
function compileInner(root: XmlElem, opts: CompileOptions, debug: boolean): CompileResult {
  try {
    const seenIncludes = new Set<string>();
    AUTO_ORIGINS = new Set<string>();
    LIBRARY_ORIGINS = new Set<string>();
    ORIGIN_RANK = new Map<string, number>();
    ORIGIN_RANK_NEXT = 0;
    ...
    for (const k of Object.keys(defs)) cattrs[k] = emitTyped(defs[k]);
    ...
    for (const child of root.children) {
      if (child.type === "elem" && (child.name === "class" || child.name === "interface")) {
        ...
        if (!name) throw new Unsupported(`<${child.name}> without name`);
        classes.set(name, { name, superTag: child.attrs["extends"] || "view", el: child });
        if (child.name === "interface") interfaces.add(name);
      }
    }
    const resolve: Resolve = (tag) => {
      if (classes.has(tag)) return classJsName(tag);
    ...
    const declaredType = (className: string, attr: string): string | null | undefined => {
      const def = classes.get(className);
      if (!def) return undefined; // not a user class
    ...
    const inheritedWhen = (className: string, attr: string): string | undefined => {
    ...
    const schemaTypeOf = (tag: string, attr: string): string | null => {
      let cur: string | undefined = tag;
      let sawUntyped = false;
      while (cur && classes.has(cur)) {
        const t = declaredType(cur, attr);
        if (t === null) sawUntyped = true; // declared here but untyped; try parent
    ...
    const resolveConstraintType = (tag: string, attr: string, parentTag?: string): string => {
    ...
    const valueTypeOf = (tag: string, attr: string): AttrType => {
      const t = schemaTypeOf(tag, attr);
    ...
    const declaresMemberHere = (className: string, name: string): boolean => {
      const def = classes.get(className);
      if (!def) return false;
      for (const c of def.el.children) {
    ...
    const isInherited = (superTag: string, attr: string): boolean => {
      let cur: string | undefined = superTag;
      while (cur && classes.has(cur)) {
        if (declaresMemberHere(cur, attr)) return true;
        cur = classes.get(cur)!.superTag;
      }
    ...
    const isStateClass = (tag: string): boolean => {
      let cur: string | undefined = tag;
      const seen = new Set<string>();
      while (cur && !seen.has(cur)) {
        ...
        if (!def) return false; // a non-state built-in base
        cur = def.superTag;
      }
      return false;
    };
    ...
    const isInputTextTag = (tag: string): boolean => {
      let cur: string | undefined = tag;
      const seen = new Set<string>();
      while (cur && !seen.has(cur)) {
        if (cur === "inputtext") return true;
        seen.add(cur);
        const def = classes.get(cur);
        if (!def) return false;
        cur = def.superTag;
    ...
    const hasTextContent = (tag: string): boolean => schemaTypeOf(tag, "text") != null;
    ...
    const childCountMemo = new Map<string, number>();
    const classDefChildCount = (tag: string): number => {
      if (childCountMemo.has(tag)) return childCountMemo.get(tag)!;
      if (interfaces.has(tag)) return 0; // an interface contributes no default children
      const def = classes.get(tag);
      if (!def) return 0; // built-in: no LZX default children
    ...
    const inheritsMemo = new Map<string, boolean>();
    const inheritsChildren = (tag: string): boolean => {
      if (inheritsMemo.has(tag)) return inheritsMemo.get(tag)!;
      if (!classes.has(tag)) return false; // built-in
    ...
    const effectiveInitstage = (el: XmlElem): string | undefined => {
      const own = el.attrs["initstage"];
      if (own != null) return own;
      let cur: string | undefined = el.name;
      while (cur && classes.has(cur) && storedCount.has(cur)) {
        const cdef: ClassDef = classes.get(cur)!;
        const ci: string | undefined = cdef.el.attrs["initstage"];
        if (ci != null) return ci;
        cur = cdef.superTag;
      }
      return undefined;
    };
    const instanceContribution = (el: XmlElem): number => {
      if (isStateClass(el.name)) return 1; // parentClassModel.isstate → delayed
      ...
      if (initstage === "late" || initstage === "defer") return 0;
      ...
      if (isLiteralDatasetEl(el)) return n;
      for (const c of childViews(el)) n += instanceContribution(c); // the node's own children
      return n;
    };
    const classStoredCount = (def: ClassDef): number => {
      let n = classes.has(def.superTag) ? storedCount.get(def.superTag) ?? 1 : 1;
      for (const c of childViews(def.el)) n += instanceContribution(c);
      return n;
    };
    ...
    const declaredResources = new Set<string>(); // all <resource name=…> names (known up-front)
    const emittedResources = new Set<string>(); // <resource name=…> declarations already emitted
    // Anonymous `$LZ` resource definitions from inline `resource="path"` view
    // references. The oracle resolves these during VIEW compilation — its final
    // phase — so they are emitted AFTER every named `<resource name=…>` declaration
    // and every `<font>` (which are compiled in the earlier schema phase and append
    // to `mResourceDefs` first), regardless of document order. So we keep named
    // resources + fonts inline in `preamble` (advancing the shared `spriteOffset` as
    // they go), DEFER the anon entries here in document/registration order, and flush
    // them after all named/font preamble is built — continuing `spriteOffset` from
    // its final named value. `$LZ` NAMES are still assigned at registration time
    // (document order), so only the emission POSITION + sprite offset move.
    type AnonResEntry = { height: number; render: (off: number) => string };
    const anonResEntries: AnonResEntry[] = [];
    ...
    const fontEntries: { line: string; origin: string }[] = [];
    let spriteOffset = 0;
    const registerResource = (ref: string, originId?: string): string => {
      // A reference to a declared <resource name=…> resolves to that name directly.
      if (declaredResources.has(ref)) return ref;
      const info = opts.resolveResource?.(ref, originId);
      if (!info) throw new Unsupported(`unresolved resource: ${ref}`);
      ...
      anonResEntries.push({
        ...
        render: (off) =>
          `LzResourceLibrary.${name}={ptype:${jsString(info.ptype)},frames:['${info.relPath}']` +
    ...
    const registerNamedResource = (el: XmlElem): void => {
      const name = el.attrs["name"];
      if (!name) throw new Unsupported(`<resource> without name`);
      if (emittedResources.has(name)) return;
      const frameRefs: string[] = [];
      for (const c of el.children) {
        if (c.type === "text") { if (c.value.trim()) throw new Unsupported(`text in <resource>`); continue; }
        if (c.name === "frame") {
          const src = c.attrs["src"];
          if (!src) throw new Unsupported(`<frame> without src`);
          frameRefs.push(src);
        } else throw new Unsupported(`<${c.name}> in <resource>`);
      }
      ...
      let infos: ResourceInfo[];
      if (enumerated) {
        infos = enumerated;
      } else {
        if (srcAttr) frameRefs.unshift(srcAttr);
        if (frameRefs.length === 0) throw new Unsupported(`<resource> without frames`);
        infos = frameRefs.map((r) => {
          const i = opts.resolveResource?.(r, el.origin);
          if (!i) throw new Unsupported(`unresolved resource: ${r}`);
          return i;
        });
      }
      ...
      const cell = (i: ResourceInfo, dim: number): number => {
        if (!/\.gif$/i.test(i.relPath)) return dim;
    ...
    const compileFontTag = (el: XmlElem): void => {
      const fontOrigin = el.origin ?? "";
      const name = el.attrs["name"];
      if (!name) throw new Unsupported(`<font> without name`);
      if (el.attrs["device"] === "true") throw new Unsupported(`device <font>`);
      const emitFace = (face: XmlElem): void => {
        const src = face.attrs["src"];
        if (!src) return;
        const info = opts.resolveFont?.(src);
        if (!info) throw new Unsupported(`unresolved font: ${src}`);
        let style = face.attrs["style"] || "";
        if (style === "") style = "plain";
        let weight = "normal";
        if (style === "plain") { weight = "normal"; style = "normal"; }
        else if (style === "bold") { weight = "bold"; style = "normal"; }
        else if (style === "italic") { weight = "normal"; style = "italic"; }
        else if (style === "bold italic" || style === "bolditalic") { weight = "bold"; style = "italic"; }
        fontEntries.push({ line: `LzFontManager.addFont('${name}','${style}','${weight}','${info.relPath}','${info.ptype}');`, origin: fontOrigin });
      };
      if (el.attrs["src"]) emitFace(el);
      for (const c of el.children) {
        if (c.type === "text") { if (c.value.trim()) throw new Unsupported(`text in <font>`); continue; }
        if (c.name === "face") emitFace(c);
        else throw new Unsupported(`<${c.name}> in <font>`);
      }
    };

    const ctx: Ctx = { resolve, resolveConstraintType, valueTypeOf, isInherited, mGen, globals, globalOrigins, registerResource, isStateClass, inheritedWhen, hasTextContent, isInputTextTag, classes, interfaces };
    ...
    let lastTopInstance: BuiltNode | undefined;
    ...
    const compileClass = (name: string): string => {
      if (!classes.has(name)) return ""; // built-in / unknown — not LZX-emitted here
      if (interfaces.has(name)) return ""; // an <interface> emits nothing
    ...
    const emitClassDef = (def: ClassDef): string => {
      const child = def.el;
      ...
      let lastMemberConBody: string | undefined;
      let lastMemberConSrcLine: number | undefined;
      ...
      let placementAttrSrcLine: number | undefined;
      ...
      let lastStaticMethodClose: number | undefined;
      const noteMember = (el: XmlElem, handler: boolean) => {
      ...
      const classAllocEntries: string[] = [];
      ...
      for (const an of child.attrOrder) {
        if (an === "name" || an === "extends") continue;
        const raw = child.attrs[an];
        let con: { when: string; expr: string; literal: boolean } | null =
          parseConstraint(raw) && { ...parseConstraint(raw)!, literal: false } || null;
        ...
        let pctType: string | null = null;
        if (!con && raw.trim().endsWith("%")) {
          try { pctType = resolveConstraintType(def.name, an); } catch { pctType = null; }
          const pexpr = percentConstraintExpr(pctType, an, raw);
          if (pexpr) con = { when: "", expr: pexpr, literal: false };
        }
        // A plain value on an attribute whose INHERITED declaration is
        // `when="once|always"` becomes a literal constant constraint with that
        ...
        if (!con && !isEventAttr(an)) {
          ...
          if (iw === "once" || iw === "always") con = attrConstraint(raw, iw);
        }
        if (isEventAttr(an)) {
          ...
          if (COMPILE_DEBUG && "lastBody" in c) {
            ...
            lastMemberConBody = (c as any).lastBody; lastMemberConSrcLine = (c as any).lastSrcLine;
          }
          ...
          if ("plain" in cv) defaultAttrs[an] = btNoteColorInit(cv.plain, child.endLine ?? child.line ?? 0);
          else {
            instEntries.push(...cv.entries); defaultAttrs[an] = btNoteConstraintInit(cv.init, child.endLine ?? child.line ?? 0, debugFile(child));
            ...
            if (COMPILE_DEBUG && cv.cc.lastBody !== undefined) {
              noteMember(child, false);
              lastMemberConBody = cv.cc.lastBody; lastMemberConSrcLine = cv.cc.lastSrcLine;
            }
          }
        } else {
      ...
      let classDatapath: BuiltNode | undefined;
      for (const c of child.children) {
        if (c.type === "text") continue; // stray text in a class body is ignored
        if (c.name === "doc") continue;
        if (c.name === "handler") {
        ...
        if (c.name === "attribute") {
          ...
          if (an === "defaultplacement") placementAttrSrcLine = c.line ?? c.endLine ?? placementAttrSrcLine;
          const raw = "value" in c.attrs ? c.attrs["value"] : null;
          ...
          if (c.attrs["style"] != null) {
            ...
            if (c.attrs["setter"] != null) noteMember(c, false);
            ...
            noteMember(c, false);
            lastMemberConBody = cc.lastBody; lastMemberConSrcLine = cc.lastSrcLine;
          } else {
            instEntries.push(...slot, ...setterEntry);
            if (c.attrs["setter"] != null) noteMember(c, false);
            if (raw != null) {
          ...
          const en = c.attrs["name"];
          if (!en) throw new Unsupported(`<event> without name`);
          if (!isInherited(def.superTag, en)) instEntries.push(voidSlot(en));
          ...
          if (classDatapath) throw new Unsupported(`multiple <datapath> children`);
          classDatapath = buildNode(c, ctx, false, null, def.name);
          if (classDatapath.datapath) throw new Unsupported(`nested <datapath>`);
      ...
      if (!isStateClass(def.name)) {
        for (const c of childViews(child)) {
          if (!isStateClass(c.name)) continue;
          for (const h of collectNamedChildren(childViews(c), c.name, ctx))
      ...
      let defaultPlacementTarget: string | undefined;
      ...
      const emitEntries = isStateClass(def.name)
        ...
        : instEntries;
      let dbg: DebugClassCtx | undefined;
      if (COMPILE_DEBUG) {
        ...
        if (stateClass && lastOwnChild !== undefined) {
          for (const cn of childNodes) if (cn.subtreeLastLiteralLine != null) { stateChildRich = true; break; }
        }
        ...
        if (childOnlyRich && defaultPlacementTarget === undefined) {
          ...
          for (const cn of childNodes) if (cn.subtreeLastLiteralLine != null) { childOnlyNoLiteral = false; break; }
        }
        ...
        if (stateChildRich) {
          ...
          let lastLit: number | undefined;
          for (const cn of childNodes) if (cn.subtreeLastLiteralLine != null) lastLit = cn.subtreeLastLiteralLine;
          ...
          for (let k = emitEntries.length - 1; k >= 0 && isVoidSlot(emitEntries[k]); k--) trailingVoidSlots++;
      ...
      if (DEBUG_STMTS) {
        lastTopFile = debugFile(child); lastTopWasInstance = false;
    ...
    const memberEls = root.children.filter(
      (c): c is XmlElem => c.type === "elem" && CANVAS_MEMBER_TAGS.has(c.name)
    );
    ...
    const canvasEventAttrs: Record<string, string> = {};
    const canvasEventOrder: string[] = [];
    const canvasAttrLines: Record<string, number> = {};
    for (const name of root.attrOrder) {
      if (isEventAttr(name)) {
    ...
    if (memberEls.length > 0 || canvasEventOrder.length > 0) {
      const synth: XmlElem = { type: "elem", name: "canvas", attrs: canvasEventAttrs, attrOrder: canvasEventOrder, attrLines: canvasAttrLines, children: memberEls, origin: root.origin, line: root.line, endLine: root.endLine };
      ...
      if (built.delegateList.length > 0) attrs["$delegates"] = "[" + built.delegateList.join(COMPILE_DEBUG ? ", " : ",") + "]";
      for (const s of built.attrSlots) if (!(s in attrs)) attrs[s] = "void 0";
      for (const [k, v] of Object.entries(attrs)) cattrs[k] = v;
      ...
      if (hasMethods) {
        ...
        if (debug) canvasAnonDefDebug = emitAnonClassDebug(canvasClass, "LzCanvas", "canvas", built.methodEntries, null, built);
      }
    }
    ...
    for (const child of root.children)
      if (child.type === "elem" && child.name === "resource") {
        ...
        if (rn) declaredResources.add(rn);
      }
    let js = "";
    for (const child of root.children) {
      // Canvas-level members are compiled into the anon class above, not here.
      if (child.type === "elem" && CANVAS_MEMBER_TAGS.has(child.name)) continue;
      if (child.type === "text") {
        if (child.value.trim()) throw new Unsupported(`text directly under <canvas>`);
      ...
      if (child.name === "interface") continue;
      ...
      if (child.name === "script" && child.attrs["when"] === "immediate") {
        ...
        if (ssrc) {
          ...
          if (stext == null) throw new Unsupported(`<script src="${ssrc}"> not found`);
          body = `#file ${ssrc}\n#line 1\n` + stext;
        }
        if (DEBUG_STMTS) {
          for (const u of compileProgramDebug(body, debugFile(child), bodyLineOf(child))) pushDebug(u);
          lastTopWasInstance = false; // an immediate-script body ends on generated context
          continue;
        }
        const prog = compileProgram(body);
        if (prog) js += prog.endsWith(";") ? prog : prog + ";";
        continue;
      }
      ...
      if (child.name === "security") continue;
      ...
      if (child.name === "stylesheet") {
        if (child.attrs["src"]) throw new Unsupported(`<stylesheet src=…>`);
        ...
        if (DEBUG_STMTS) {
          ...
          if (progD) {
            ...
            for (const u of compileStylesheetDebug(progD, debugFile(child), styleLine, styleCol)) pushDebug(u);
        ...
        if (prog) js += ";" + compileProgram(prog);
        continue;
      }
      ...
      if (child.name === "dataset" && isLocalDataset(child)) {
        if (DEBUG_STMTS) {
          ...
          const dsDir: [string, number] = debugAnonPrev ? ["", 0] : [crossUnitFile, 1];
          for (const u of compileDatasetDebug(child, globals, globalOrigins, opts, dsDir)) pushDebug(u);
      ...
      if (child.name === "debug") {
        if (debug) {
          const dbgEl: XmlElem = { ...child, name: "LzDebugWindow" };
    ...
    if (DEBUG_STMTS) {
      ...
      const pushReg = (marker: string) => { pushDebug(regResetPrefix + marker); regResetPrefix = ""; };
      registrations.forEach((reg, i) =>
        pushReg(registerReg({ body: reg.replace(/;$/, "").replace("]=", "] = "), file: regFile, seq: i + 1 }))
      );
    ...
    const fontCat = (o: string): number => (AUTO_ORIGINS.has(o) ? 0 : LIBRARY_ORIGINS.has(o) ? 2 : 1);
    preamble.push(
      ...fontEntries
