...
public class NodeModel implements Cloneable {
  ...
  static class CompiledAttribute {
    ...
    // ORACLE FIX #3 (LPP-3949): in DEBUG_BACKTRACE builds, anchor a synthesized
    // class-default CONSTRAINT init-expr (new LzOnceExpr/LzAlwaysExpr/
    // LzConstraintExpr/LzStyleConstraintExpr) to its attribute's DECLARATION
    // source line by wrapping it in the same #beginAttribute/srcloc/#endAttribute
    // directive the WHEN_IMMEDIATELY path already emits below. Without this the
    // backtrace noteCallSite ($lzsc$a.lineno = node.beginLine) records the running
    // JavaCC physical line of the generated mergeAttributes object instead of the
    // declaration line. Gated on BACKTRACE_PROPERTY so it is INERT for normal
    // (non-backtrace) debug and production builds (those never noteCallSite these
    // inits, and the directive would otherwise perturb their #file/#line state).
      ...
      if (when.equals(WHEN_PATH) || (when.equals(WHEN_STYLE)) || when.equals(WHEN_ONCE) || when.equals(WHEN_ALWAYS)) {
        ...
        if (when.equals(WHEN_ONCE) || when.equals(WHEN_PATH)) {
          ...
          if (constantValue) {
            ...
            return new BindingExpr(btAnchorConstraintInit("new " + kind + "(" +
                                   ScriptCompiler.quote(name) + ", " +
    ...
    void addEventElement(Element element) {
        ...
        try {
            ...
            throw new CompilationError(
                 ...
                 */
                org.openlaszlo.i18n.LaszloMessages.getMessage(
                    NodeModel.class.getName(),"051018-1157", new Object[] {tagName})
                , element);
        }
   ...
   */
    Map asMap(CompilationEnvironment cEnv) {
      ...
      if (hasMethods) {
        ...
        classModel = new ClassModel(ViewSchema.KIND_INSTANCE_CLASS, tagName, false, schema, element, cEnv);
