...
import org.openlaszlo.sc.parser.Token;
...
abstract class ToplevelCompiler extends ElementCompiler {
  ...
  public void outputTagMap(CompilationEnvironment env) {
      ...
      if (tagmap.length() > 0) {
          // PATCH (2026-06-24): the tag->class map is compiler-GENERATED code with
          // no source file. Token.currentPathname is a static that survives across
          // compileScript() calls, so without this reset the generated tag map
          // inherits whatever source file the last instance's last-parsed script
          // fragment happened to leave behind -- a debug-only leak that makes the
          // registration trailer's #file source-location attribution depend on
          // internal parse order rather than anything real. Resetting to "" gives
          // the generated map a consistent (file-less) attribution. Debug-only;
          // production output is unaffected. See modern-build/oracle/patch/README.md.
          Token.setCurrentPathname("");
